...
final class RNXProtocolTests: XCTestCase {
  ...
  func testRequestPackStdinData() throws {
    ...
    guard case .array(let arr) = try MsgPack.decode(req.pack()),
      case .bytes(let stdinBytes) = arr[4]
    else {
