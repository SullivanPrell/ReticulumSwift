...
final class TransportPersistenceTests: XCTestCase {
  ...
  func testKnownDestinationsSurviveRestart() throws {
    ...
    XCTAssertEqual(
      recalled?.publicKeyBytes, id.publicKeyBytes,
  ...
  func testPacketHashlistSurvivesRestart() throws {
    ...
    XCTAssertTrue(
      rns1.transport.testContainsPacketHash(fakeHash),
    ...
    XCTAssertTrue(
      rns2.transport.testContainsPacketHash(fakeHash),
  ...
  func testExpiredPathsNotRestoredOnStart() throws {
    ...
    XCTAssertTrue(
      rns2.transport.hasPath(to: liveHash),
