...
final class PathTableParityTests: XCTestCase {
  ...
  private func makeFixture(
    ...
    return Fixture(
      transport: transport, interface: iface, identity: identity,
  ...
  func testDestinationTableIsReferenceShape() throws {
    ...
    XCTAssertEqual(
      fields[0], .bytes(fixture.destinationHash),
    ...
    guard case .double(let timestamp) = fields[1] else {
      return XCTFail(
        "field 1 is `de[IDX_PT_TIMESTAMP]`, a unix timestamp as written by "
    ...
    XCTAssertEqual(
      fields[2], .bytes(Data(repeating: 0xAB, count: 16)),
      """
    ...
    XCTAssertEqual(
      fields[5], .array([.bytes(Data(repeating: 0x11, count: 10))]),
      "field 5 is the random-blob list, raw bytes per blob (Transport.py:3394)")

    XCTAssertEqual(
      fields[6], .bytes(fixture.interface.hash),
      """
    ...
    XCTAssertEqual(
      fields[7], .bytes(fixture.announceHash),
      """
  ...
  func testAnnounceHashResolvesInTheCache() throws {
    ...
    guard case .array(let entries) = decoded, case .array(let fields) = entries.first,
      fields.count == 8, case .bytes(let announceHash) = fields[7]
    else {
    ...
    XCTAssertNotNil(
      ...
      """)
    XCTAssertEqual(
      try cached?.pack(), try fixture.announce.pack(),
  ...
  func testRestoreRebuildsThePathFromTheCachedAnnounce() throws {
    ...
    let restored = try XCTUnwrap(
      receiver.paths[fixture.destinationHash],
      "the entry must come back")
    XCTAssertEqual(restored.hops, 4)
    XCTAssertEqual(
      restored.nextHopInterface?.hash, iface.hash,
    ...
    XCTAssertEqual(
      restored.randomBlobs, [Data(repeating: 0x22, count: 10)],
      "replay protection survives the restart")
    XCTAssertEqual(
      restored.cachedAnnounceHash, fixture.announceHash,
  ...
  func testRestoredAnnounceHopCountIsIncremented() throws {
    ...
    let restored = try XCTUnwrap(
      PathStore.restoredAnnounce(for: entry, from: receiver),
      """
    ...
    XCTAssertEqual(
      ...
      """)
    XCTAssertEqual(
      Hashes.fullHash(try restored.hashablePart()), fixture.announceHash,
  ...
  func testEntryCarriesNoIdentityMaterial() throws {
    ...
    XCTAssertNil(
      raw.range(of: Data(fixture.identity.publicKeyBytes.hexString.utf8)),
      explanation)
  }
}
