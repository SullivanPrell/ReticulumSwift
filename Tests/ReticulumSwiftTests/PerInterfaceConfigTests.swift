...
final class PerInterfaceConfigTests: XCTestCase {
  ...
  private func synthesise(_ body: String) throws -> any Interface {
    ...
    return try XCTUnwrap(
      reticulum.transport.interfaces.first,
  ...
  func testIngressControlCanBeDisabled() throws {
    ...
    XCTAssertTrue(
      try synthesise("    listen_port = 4970").ingressControl,
  ...
  func testConfiguredIngressThresholdChangesTheLimitingDecision() throws {
    ...
    XCTAssertFalse(
      reticulum.transport.shouldIngressLimit(on: tolerant, now: base + 1.0),
      """
      ...
      """)
    XCTAssertTrue(
      reticulum.transport.shouldIngressLimit(on: twitchy, now: base + 1.0),
  ...
  func testAnnouncesFromInternalCanBeDisabled() throws {
    ...
    XCTAssertTrue(
      try synthesise("    listen_port = 4971").announcesFromInternal,
