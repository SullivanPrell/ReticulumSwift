...
final class LinkConstantsParityTests: XCTestCase {
  ...
  func testKeepAliveTimeoutFactor() {
    XCTAssertEqual(
      Link.staleTime / Link.keepaliveInterval, 2,
  ...
  func testEstablishedLinkDerivedKeyIs64Bytes() throws {
    ...
    let aI = LinkConstLoopback(name: "A")
    let bI = LinkConstLoopback(name: "B")
    aI.paired = bI
    bI.paired = aI
    ...
    XCTAssertEqual(
      initiator.derivedKey?.count, 64,
      "initiator derivedKey must be 64 bytes (AES-256-CBC)")
    XCTAssertEqual(
      responder.derivedKey?.count, 64,
      "responder derivedKey must be 64 bytes (AES-256-CBC)")
    XCTAssertEqual(
      initiator.derivedKey, responder.derivedKey,
  ...
  func testEstablishedLinkAES256EncryptDecryptRoundTrip() throws {
    ...
    bT.ownerIdentity = bId
    bT.register(destination: bDest)
    let aI = LinkConstLoopback(name: "C")
    let bI = LinkConstLoopback(name: "D")
    aI.paired = bI
    bI.paired = aI
    aT.register(interface: aI)
    bT.register(interface: bI)
    ...
    XCTAssertGreaterThanOrEqual(
      ciphertext.count, plaintext.count + 48,
  ...
  func testEstablishedLinkAES256BidirectionalRoundTrip() throws {
    ...
    bT.ownerIdentity = bId
    bT.register(destination: bDest)
    let aI = LinkConstLoopback(name: "E")
    let bI = LinkConstLoopback(name: "F")
    aI.paired = bI
    bI.paired = aI
    aT.register(interface: aI)
    bT.register(interface: bI)
