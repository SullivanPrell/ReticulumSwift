...
final class LinkSafeguardsTests: XCTestCase {
  ...
  private final class LinkPair {
    let tA: Transport
    let tB: Transport
    let iA: LoopbackInterface
    let iB: LoopbackInterface
    let dst: Destination
    let initiator: Link
    var responder: Link?
    init(
      tA: Transport, tB: Transport, iA: LoopbackInterface, iB: LoopbackInterface,
  ...
  private func makeLinkPair() throws -> LinkPair {
    ...
    let tA = Transport()
    let tB = Transport()
    let iA = LoopbackInterface(name: "A")
    let iB = LoopbackInterface(name: "B")
    iA.paired = iB
    iB.paired = iA
    tA.register(interface: iA)
    tB.register(interface: iB)
    ...
    return LinkPair(
      tA: tA, tB: tB, iA: iA, iB: iB, dst: dst, initiator: link, responder: responderLink)
  }
  ...
  func testRemoteIdentifyAppliedOnlyOnce() throws {
    ...
    let dstIdentity = Identity()
    let dst = try Destination(
      identity: dstIdentity, direction: .in, kind: .single, appName: "test", aspects: [])
    let tA = Transport()
    let tB = Transport()
    let iA = LoopbackInterface(name: "A")
    let iB = LoopbackInterface(name: "B")
    ...
    guard let rLink = responderLink else {
      XCTFail("no responder link")
      return
    }
    XCTAssertEqual(identifyCount, 1, "remote_identified must fire exactly once")
    XCTAssertEqual(
      rLink.remoteIdentity?.publicKeyBytes, identityA.publicKeyBytes,
  ...
  func testMalformedResourceAdvertisementTearsDownLink() throws {
    ...
    let link = pair.initiator
    XCTAssertEqual(link.status, .active)
    guard let rLink = pair.responder else {
      XCTFail("no responder link")
      return
    }
