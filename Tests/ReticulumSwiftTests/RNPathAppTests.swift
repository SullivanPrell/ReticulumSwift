...
private let hashA = Data([
  0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x00, 0x11,
  0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99,
])
private let hashB = Data([
...
final class RNPathAppConstantsTests: XCTestCase {
  ...
  func testSpinnerGlyphs() {
    ...
    XCTAssertEqual(
      RNPathApp.spinnerSymbols, Array("\u{2884}\u{2882}\u{2881}\u{2841}\u{2848}\u{2850}\u{2860}"))
  }
...
final class RNPathParseTests: XCTestCase {
  ...
  func testParseHashWording() {
    ...
    XCTAssertThrowsError(try RNPathApp.parseHash("")) { error in
      ...
      XCTAssertEqual(
        (error as? RNPathApp.ParseError)?.message,
  ...
  func testParseDestinationWording() {
    ...
    XCTAssertThrowsError(try RNPathApp.parseDestination(String(repeating: "g", count: 32))) {
      error in
      XCTAssertEqual(
        (error as? RNPathApp.ParseError)?.message,
  ...
  func testLeadingPlusIsRejected() {
    ...
    XCTAssertThrowsError(try RNPathApp.parseHash("+a" + String(repeating: "0", count: 30))) {
      error in
      XCTAssertEqual(error as? RNPathApp.ParseError, .invalidHash)
    }
...
final class RNPathPathLineTests: XCTestCase {
  ...
  private func entry(hops: UInt8, via: Data? = hashB) -> RNPathTableEntry {
    RNPathTableEntry(
      destinationHash: hashA, timestamp: 1_700_000_000, via: via,
  ...
  func testStableSortByInterfaceThenHops() {
    let a = RNPathTableEntry(
      destinationHash: hashA, timestamp: 0, via: nil, hops: 3,
      expires: 0, interfaceName: "BInterface")
    let b = RNPathTableEntry(
      destinationHash: hashB, timestamp: 0, via: nil, hops: 1,
      expires: 0, interfaceName: "BInterface")
    let c = RNPathTableEntry(
      destinationHash: hashC, timestamp: 0, via: nil, hops: 9,
  ...
  func testSortIsStableWithinAGroup() {
    let entries = (0..<5).map { index in
      RNPathTableEntry(
        destinationHash: Data([UInt8(9 - index)] + Array(repeating: 0, count: 15)),
        timestamp: 0, via: nil, hops: 2, expires: 0, interfaceName: "Iface")
    }
    XCTAssertEqual(
      RNPathTableEntry.sortedForDisplay(entries).map(\.destinationHash),
...
final class RNPathRateLineTests: XCTestCase {
  ...
  func testRateViolationPluralisation() {
    XCTAssertFalse(
      RNPathFormatter.rateLine(entry(violations: 0), now: now)!
        .contains("active rate violation"))
    XCTAssertTrue(
      RNPathFormatter.rateLine(entry(violations: 1), now: now)!
        .hasSuffix(", 1 active rate violation"))
    XCTAssertTrue(
      RNPathFormatter.rateLine(entry(violations: 2), now: now)!
        .hasSuffix(", 2 active rate violations"))
  }

  func testBlockedUntil() {
    XCTAssertFalse(
      RNPathFormatter.rateLine(entry(blockedUntil: now - 10), now: now)!
        .contains("new announces allowed in"))
    // Python: bli = now - (int(blocked_until) - now)—600 seconds ahead reads back as 600 seconds past.
    XCTAssertTrue(
      RNPathFormatter.rateLine(entry(blockedUntil: now + 600), now: now)!
  ...
  func testEmptyTimestampsIsSignalledRatherThanRendered() {
    ...
    XCTAssertEqual(
      RNPathFormatter.rateErrorLine(entry(timestamps: [])),
  ...
  func testSortedByLastIsStable() {
    let older = RNPathRateEntry(
      destinationHash: hashA, last: 100, rateViolations: 0,
      blockedUntil: 0, timestamps: [])
    let newer = RNPathRateEntry(
      destinationHash: hashB, last: 200, rateViolations: 0,
      blockedUntil: 0, timestamps: [])
    let tied = RNPathRateEntry(
      destinationHash: hashC, last: 100, rateViolations: 0,
      blockedUntil: 0, timestamps: [])
    XCTAssertEqual(
      RNPathRateEntry.sortedByLast([newer, older, tied]).map(\.destinationHash),
      [hashA, hashC, hashB])
  }
}
...
final class RNPathRunnerHelpGateTests: XCTestCase {
  ...
  func testGuardedFlagsSuppressHelp() {
    for mutate in [
      { (o: inout RNPathOptions) in o.table = true },
      { o in o.rates = true },
  ...
  func testUnguardedFlagsStillPrintHelp() {
    for mutate in [
      { (o: inout RNPathOptions) in o.blackhole = true },
...
final class RNPathModelCodecTests: XCTestCase {
  ...
  func testPathEntryRoundTrip() throws {
    for via in [hashB, nil] {
      let entry = RNPathTableEntry(
        destinationHash: hashA, timestamp: 1.5, via: via,
        hops: 2, expires: 9.0, interfaceName: "Iface[x]")
      let decoded = RNPathTableEntry.decode(
        try MsgPack.decode(MsgPack.encode(entry.msgpackValue())))
  ...
  func testPathEntryKeyOrderMatchesPython() {
    ...
    guard
      case .map(let pairs) = RNPathTableEntry(
        destinationHash: hashA, timestamp: 0,
  ...
  func testRateEntryRoundTrip() throws {
    let entry = RNPathRateEntry(
      destinationHash: hashA, last: 1.5, rateViolations: 2,
  ...
  func testDecodeToleratesIntegerEncodings() {
    ...
    let rate = RNPathRateEntry.decode(
      .map([
        (.string("hash"), .bytes(hashA)),
        (.string("last"), .uint(3)),
  ...
  func testBridgingFromTransportEntry() {
    ...
    let source = Transport.PathTableEntry(
      destinationHash: hashA, via: hashA, hops: 1,
...
final class RNPathBlackholeDecodeTests: XCTestCase {
  ...
  func testWireOrderIsPreserved() {
    let entries = RNPathBlackholeEntry.decodeList(
      .map([
...
final class RNPathRemotePayloadTests: XCTestCase {
  ...
  func testTablePayloadHasThreeElements() {
    ...
    let payload = RNPathRemoteClient.pathRequestPayload(
      command: "table",
      destinationHash: hashA, maxHops: 3)
    XCTAssertEqual(payload, .array([.string("table"), .bytes(hashA), .uint(3)]))

    let empty = RNPathRemoteClient.pathRequestPayload(
      command: "table",
...
final class RNPathDestinationHashTests: XCTestCase {
  ...
  func testRawHashDerivationMatchesTheDocumentedDigest() {
    for (purpose, appAspects) in [
      (RNPathRemoteClient.Purpose.management, RNPathApp.managementAspects),
      ...
      XCTAssertEqual(
        RNPathRemoteClient.destinationHash(purpose: purpose, identityHash: hashA),
        expected)
    }
  }
  ...
  func testAgreesWithTheDiscoveryBlackholeDestination() throws {
    ...
    XCTAssertEqual(
      RNPathRemoteClient.destinationHash(
        purpose: .blackhole,
...
final class RNPathJSONTests: XCTestCase {
  ...
  func testRateTableJSON() {
    let entry = RNPathRateEntry(
      destinationHash: hashA, last: 1.5, rateViolations: 0,
      blockedUntil: 0.0, timestamps: [1.0, 2.0])
    XCTAssertEqual(
      RNPathFormatter.rateTableJSON([entry]),
...
final class RNPathRemoteDecodeTests: XCTestCase {
  ...
  private var sampleTable: MsgPack.Value {
    .array([
      RNPathTableEntry(
        destinationHash: hashA, timestamp: 1, via: hashB,
        hops: 2, expires: 3, interfaceName: "i"
      ).msgpackValue()
    ])
  }
  ...
  func testDecodesRateTable() {
    let value = MsgPack.Value.array([
      RNPathRateEntry(
        destinationHash: hashA, last: 5, rateViolations: 1,
        blockedUntil: 0, timestamps: [1, 2]
      ).msgpackValue()
    ])
    XCTAssertEqual(
      RNPathRemoteClient.decodeRateTable(nativeResponse(value))?.first?.rateViolations, 1)
  }
  ...
  func testDecodesBlackholeList() {
    ...
    XCTAssertEqual(
      RNPathRemoteClient.decodeBlackholeList(nativeResponse(value))?.first?.reason, "r")
  ...
  func testTeardownReasonsMapToPythonExitCodes() {
    ...
    XCTAssertEqual(
      RNPathRemoteClient.RemoteError.pathRequestTimedOut.message, "Path request timed out")
    XCTAssertEqual(
      RNPathRemoteClient.RemoteError.linkTimedOut.message, "The link timed out, exiting now")
    XCTAssertEqual(
      RNPathRemoteClient.RemoteError.linkClosedByServer.message,
      "The link was closed by the server, exiting now")
    XCTAssertEqual(
      RNPathRemoteClient.RemoteError.linkClosedUnexpectedly.message,
...
final class RNPathRunnerTableTests: XCTestCase {
  ...
  private func fixture() -> MockManagementSource {
    ...
    management.paths = [
      RNPathTableEntry(
        destinationHash: hashA, timestamp: 0, via: hashB, hops: 3,
        expires: 0, interfaceName: "TCPInterface[b]"),
      RNPathTableEntry(
        destinationHash: hashB, timestamp: 0, via: hashB, hops: 1,
        expires: 0, interfaceName: "TCPInterface[b]"),
      RNPathTableEntry(
        destinationHash: hashC, timestamp: 0, via: hashB, hops: 5,
  ...
  func testUnmatchedFilterReportsNoPathKnown() {
    ...
    XCTAssertEqual(
      makeRunner(options, management: fixture(), recorder: recorder).run(),
      .generalFailure)
  ...
  func testEmptyRemoteTableIsTreatedAsAFailure() {
    ...
    let runner = makeRunner(
      options, management: fixture(), recorder: recorder,
  ...
  func testRemoteTableIsNotResorted() {
    ...
    options.table = true
    let recorder = OutputRecorder()
    let response = MsgPack.encode(
      .array([
        RNPathTableEntry(
          destinationHash: hashA, timestamp: 0, via: hashB, hops: 9,
          expires: 0, interfaceName: "ZInterface"
        ).msgpackValue(),
        RNPathTableEntry(
          destinationHash: hashB, timestamp: 0, via: hashB, hops: 1,
          expires: 0, interfaceName: "AInterface"
...
final class RNPathRunnerRatesTests: XCTestCase {
  ...
  private func entry(_ hash: Data, last: TimeInterval) -> RNPathRateEntry {
    RNPathRateEntry(
      destinationHash: hash, last: last, rateViolations: 0, blockedUntil: 0,
  ...
  func testOutputIsAscendingByLast() {
    ...
    XCTAssertEqual(
      makeRunner(options, management: management, recorder: recorder, now: now).run(), .ok)
  ...
  func testEmptyTableExitsZero() {
    ...
    let recorder = OutputRecorder()
    XCTAssertEqual(
      makeRunner(options, management: MockManagementSource(), recorder: recorder).run(), .ok)
  ...
  func testFilteredToNothingExitsOne() {
    ...
    XCTAssertEqual(
      makeRunner(options, management: management, recorder: recorder, now: now).run(),
      .generalFailure)
    XCTAssertEqual(recorder.lines, ["No information available"])
  }
  ...
  func testEmptyTimestampsDoesNotAbortTheLoop() {
    ...
    management.rates = [
      RNPathRateEntry(
        destinationHash: hashA, last: base - 5, rateViolations: 0,
        blockedUntil: 0, timestamps: []),
    ...
    XCTAssertEqual(recorder.lines.count, 3)
    XCTAssertEqual(
      recorder.lines[0], "Error while processing entry for <aabbccddeeff00112233445566778899>")
  ...
  func testRemoteRatesPayloadHasNoMaxHops() {
    ...
    let response = MsgPack.encode(
      .array([entry(hashA, last: now.timeIntervalSince1970).msgpackValue()]))
    let runner = makeRunner(
      options, management: MockManagementSource(), recorder: recorder,
      remoteLinkPresent: true,
      remoteRequest: { _, value in
        seen = value
        return response
      },
      now: now)
...
final class RNPathRunnerDropTests: XCTestCase {
  ...
  func testDropPathSuccessAndFailure() {
    for (result, expectedLine, expectedCode) in [
      ...
      XCTAssertEqual(
        makeRunner(options, management: management, recorder: recorder).run(),
        expectedCode)
  ...
  func testDropAnnounceQueuesPrintsBeforeCalling() {
    ...
    let runner = RNPathRunner(
      options: options, management: management,
  ...
  func testDropModesRejectABadDestinationWithExitOne() {
    for mutate in [
      { (o: inout RNPathOptions) in o.drop = true },
      { o in o.dropVia = true },
    ] {
      var options = RNPathOptions()
      mutate(&options)
      ...
      let recorder = OutputRecorder()
      XCTAssertEqual(
        makeRunner(options, management: MockManagementSource(), recorder: recorder).run(),
        .generalFailure)
      XCTAssertEqual(
        recorder.lines,
        ["Destination length is invalid, must be 32 hexadecimal characters (16 bytes)."])
...
final class RNPathRunnerBlackholeTests: XCTestCase {
  ...
  func testListingRendersAndFilters() {
    ...
    XCTAssertEqual(
      makeRunner(options, management: management, recorder: filtered, now: now).run(), .ok)
  ...
  func testFetchFailureExitsTwenty() {
    ...
    XCTAssertEqual(
      ...
      .setupFailure)
    XCTAssertTrue(
      recorder.lines[0].hasPrefix("Could not get blackholed identities from RNS instance: "))
  }
  ...
  func testCombinedFlagsFetchLocallyButFilterOnTheSecondPositional() {
    ...
    var fetched = false
    let runner = makeRunner(
      options, management: management, recorder: recorder,
      blackholeListFetch: {
        fetched = true
        return []
      }, now: now)
...
final class RNPathArgumentParsingTests: XCTestCase {
  ...
  private func makeParser() -> ArgumentParser {
    ...
    parser.option(
      ["-R"], metavar: "hash", help: "transport identity hash of remote instance to manage")
    ...
    parser.flag(
      ["-p", "--blackholed-list"],
      help: "view published blackhole list for remote transport instance")
