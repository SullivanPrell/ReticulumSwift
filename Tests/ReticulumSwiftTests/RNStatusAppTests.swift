...
final class RNStatusAppTests: XCTestCase {
  ...
  func testRemoteManagementConstants() {
    ...
    XCTAssertEqual(
      ([RNStatusApp.remoteManagementAppName] + RNStatusApp.remoteManagementAspects).joined(
        separator: "."),
  ...
  func testSortTokens() {
    ...
    XCTAssertEqual(
      RNStatusApp.Sort.allCases.map(\.rawValue),
  ...
  func testEveryFlagSpellingParses() throws {
    ...
    for (short, long) in [
      ("-a", "--all"), ("-A", "--announce-stats"), ("-P", "--pr-stats"),
  ...
  func testHelpTextMatchesArgparse() {
    ...
    XCTAssertTrue(
      lines.contains("  -D                    show details and config entries for discovered"))
    ...
    for line in lines where line.hasPrefix("  -") && line.count > 24 {
      ...
      if line[column] != " " {
        XCTAssertTrue(
          line.count <= 24 || line.hasPrefix("  -I seconds") || line.hasPrefix("  -s SORT"),
