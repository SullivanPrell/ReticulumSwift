...
final class BlackholePersistenceWiringTests: XCTestCase {
  ...
  func testPersistBlacklistSkipsExternalSourceEntries() throws {
    ...
    XCTAssertTrue(
      t2.isBlackholed(ownId.hash),
      "own entry must be loaded from 'local' file")
    XCTAssertFalse(
      t2.isBlackholed(extId.hash),
  ...
  func testReloadBlacklistSkipsExpiredEntries() throws {
    ...
    let expiredId = Identity()
    let validId = Identity()
    ...
    XCTAssertFalse(
      t2.isBlackholed(expiredId.hash),
      "expired entry (until in past) must not be loaded")
    XCTAssertTrue(
      t2.isBlackholed(validId.hash),
  ...
  func testReloadBlacklistLoadsPermanentEntries() throws {
    ...
    XCTAssertTrue(
      t2.isBlackholed(permanentId.hash),
  ...
  func testReticulumStartLoadsAndStopSavesBlacklist() throws {
    ...
    let r2 = Reticulum(
      configuration: Reticulum.Configuration(storagePath: storagePath, shareInstance: false))
    r2.transport.ownerIdentity = ownIdentity
    try r2.start()
    defer { r2.stop() }

    XCTAssertTrue(
      r2.transport.isBlackholed(id1.hash),
