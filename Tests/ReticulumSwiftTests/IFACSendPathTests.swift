...
final class IFACSendPathTests: XCTestCase {
  ...
  private func assertIfacWrapped(
    _ wrapped: Data,
    unwrapsTo raw: Data,
    ...
    XCTAssertEqual(
      w.count, raw.count + iface.ifacSize,
      "IFAC code bytes must be inserted", file: file, line: line)
    XCTAssertEqual(
      iface.unwrapIfac(w), raw,
  ...
  func testSerialSendAppliesIfac() throws {
    ...
    let wrapped = try XCTUnwrap(
      HDLC.FrameDecoder().feed(framed).first,
  ...
  func testKISSSendAppliesIfac() throws {
    ...
    let frame = try XCTUnwrap(
      KISS.FrameDecoder().feed(framed).first,
  ...
  func testAX25KISSSendAppliesIfac() throws {
    let mock = MockSerialPort()
    let iface = try AX25KISSInterface(
      name: "AX0", port: "/dev/null",
  ...
  func testCrossInterfaceUnwrapRequiresAMatchingConfiguredIFACSize() throws {
    ...
    XCTAssertNil(
      defaultTcp.unwrapIfac(try wrapOnKiss(defaultKiss, mock: defaultsMock)),
    ...
    configureIfacFromConfigBlock(
      on: kiss, netname: "shared-mesh", netkey: "s3cr3t",
      sizeBits: 128)
    try kiss.start()
    let tcp = TCPClientInterface(name: "T1", host: "127.0.0.1", port: 4243)
    configureIfacFromConfigBlock(
      on: tcp, netname: "shared-mesh", netkey: "s3cr3t",
      sizeBits: 128)
    ...
    let raw = try makePacket().pack()
    XCTAssertEqual(
      tcp.unwrapIfac(try wrapOnKiss(kiss, mock: mock)), raw,
  ...
  func testCrossInterfaceMismatchedNetkeyRejected() throws {
    ...
    configureIfacFromConfigBlock(
      on: tcp, netname: "shared-mesh", netkey: "wrong-key",
      sizeBits: 128)
    XCTAssertEqual(kiss.ifacSize, tcp.ifacSize, "the netkey must be the only difference")

    XCTAssertNil(
      tcp.unwrapIfac(try wrapOnKiss(kiss, mock: mock)),
