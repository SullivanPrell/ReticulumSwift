...
final class NetworkProbeValidationTests: XCTestCase {
  ...
  func testMissingFullNameMessage() {
    ...
    XCTAssertEqual(
      NetworkProbe.ValidationError.missingFullName.message,
  ...
  func testHashLengthMessage() {
    ...
    XCTAssertEqual(
      NetworkProbe.ValidationError.badHashLength.message,
  ...
  func testHashHexMessage() {
    ...
    XCTAssertEqual(
      NetworkProbe.ValidationError.badHashHex.message,
  ...
  func testValidateRunsBeforeAnyStack() {
    ...
    XCTAssertEqual(
      NetworkProbe.validate(
        options: NetworkProbe.Options(
...
final class NetworkProbeFormattingTests: XCTestCase {
  ...
  func testEffectiveTimeoutPrecedence() {
    ...
    XCTAssertEqual(
      NetworkProbe.effectiveTimeout(-5, firstHopTimeout: 6, mediumPathTimeout: 900), -5)
  }
}
...
final class NetworkProbeArgumentTests: XCTestCase {
  ...
  func testUsageErrorText() {
    XCTAssertEqual(
      NetworkProbe.Arguments.usageErrorText("unrecognized arguments: --bogus"),
      NetworkProbe.Arguments.usageText
  ...
  func testSinglePositionalPrintsHelp() {
    ...
    XCTAssertEqual(
      NetworkProbe.Arguments.parse(["rnprobe", "lxmf.delivery"]),
      .missingDestination)
  }
  ...
  func testEmptyConfigCollapsesToNil() {
    ...
    let action = NetworkProbe.Arguments.parse([
      "rnprobe", "--config", "", "lxmf.delivery", destHash,
    ])
    guard case .run(let options) = action else { return XCTFail("expected .run") }
  ...
  func testInvalidIntValue() {
    XCTAssertEqual(
      usageErrorDetail(["rnprobe", "-s", "x", "lxmf.delivery", destHash]),
  ...
  func testProbeCountGuards() {
    ...
    XCTAssertEqual(
      usageErrorDetail(["rnprobe", "-n", "0", "lxmf.delivery", destHash]),
      "argument -n/--probes: must be at least 1")
    XCTAssertEqual(
      usageErrorDetail(["rnprobe", "-n", "-1", "lxmf.delivery", destHash]),
      "argument -n/--probes: must be at least 1")
  }
  ...
  func testNegativeSizeGuard() {
    ...
    XCTAssertEqual(
      usageErrorDetail(["rnprobe", "-s", "-1", "lxmf.delivery", destHash]),
  ...
  func testNegativeWaitGuard() {
    ...
    XCTAssertEqual(
      usageErrorDetail(["rnprobe", "-w", "-1", "lxmf.delivery", destHash]),
...
final class NetworkProbeRunTests: XCTestCase {
  ...
  private func options(
    ...
    wait: TimeInterval = 0,
    verbosity: Int = 0
  ) -> NetworkProbe.Options {
    NetworkProbe.Options(
      fullName: "lxmf.delivery", destinationHexhash: hex,
  ...
  func testPathRequestAndSpinner() {
    ...
    let expectedPrefix =
      "Path to \(f.pretty) requested   "  // three trailing spaces
      + "\u{08}\u{08}\u{2884} \u{08}\u{08}\u{2882} \u{08}\u{08}\u{2881} "
    XCTAssertTrue(
      harness.output.stdout.hasPrefix(expectedPrefix),
  ...
  func testPathTimeout() {
    ...
    XCTAssertTrue(
      harness.output.stdout.hasSuffix(
  ...
  func testDeadlineExceededUses64Spaces() {
    ...
    XCTAssertTrue(
      harness.output.stdout.contains(
  ...
  func testPartialLossArithmetic() {
    ...
    for testCase in cases {
      ...
      XCTAssertTrue(
        harness.output.stdout.hasSuffix(testCase.summary),
  ...
  func testVerboseAnnotationOmitsMissingParts() {
    ...
    for (nextHop, name, mustNotContain) in cases {
      ...
      _ = harness.probe.run(options: options(f.hex, verbosity: 1))
      XCTAssertFalse(
        harness.output.stdout.contains(mustNotContain),
...
final class NetworkProbeCryptoTests: XCTestCase {
    ...
    -> (iface: CapturingInterface, output: CapturingProbeOutput)
  {
    ...
    let probe = NetworkProbe(
      network: TransportProbeNetwork(transport: stack.transport),
  ...
  func testSendReturnsNilWithoutIdentity() throws {
    let transport = Transport()
    let destination = try Destination(
      identity: nil, direction: .in, kind: .plain,
      appName: "lxmf", aspects: ["delivery"])
    let probe = NetworkProbe(transport: transport, entropy: MockProbeEntropy())
...
final class NetworkProbeProofPacketTests: XCTestCase {
  ...
  func testProofPacketRecordedWhenSupplied() throws {
    var proof = Packet(
      destinationType: .single, packetType: .proof,
      destinationHash: Data(repeating: 0x01, count: 16),
    ...
    XCTAssertEqual(
      receipt.proofPacketTruncatedHash,
...
private final class MockProbeNetwork: ProbeNetwork {
  ...
  func transmit(ciphertext: Data, to destinationHash: Data) throws -> (any ProbeReceipt)? {
    ...
    if let deliveredCount {
      return FakeProbeReceipt(
        status: transmitCalls <= deliveredCount ? .delivered : .failed,
        rtt: 0.5)
    }
    return receipt
  }
