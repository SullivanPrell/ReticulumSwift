...
final class RNStatusStatsTests: XCTestCase {
  ...
  static func named(_ name: String, _ extra: [(String, MsgPack.Value)] = [])
    -> RNStatusInterfaceStats
  {
  ...
  func testPresenceIsDistinguishableFromNil() {
    let keys = [
      "noise_floor", "cpu_load", "cpu_temp", "mem_load", "interference",
  ...
  func testAlwaysVisibleInterfaces() {
    for name in [
      "Shared Instance[37428]", "TCPInterface[Server on 0.0.0.0:4242]",
  ...
  func testBurstFilterRequiresBothKeys() {
    ...
    let bursting = Self.named(
      "X", [("burst_active", .bool(true)), ("pr_burst_active", .bool(false))])
    XCTAssertTrue(RNStatusStats.passesFilters(bursting, nameFilter: nil, burstFilter: true))

    let prBursting = Self.named(
      "X", [("burst_active", .bool(false)), ("pr_burst_active", .bool(true))])
    XCTAssertTrue(RNStatusStats.passesFilters(prBursting, nameFilter: nil, burstFilter: true))

    let quiet = Self.named(
      "X", [("burst_active", .bool(false)), ("pr_burst_active", .bool(false))])
  ...
  private func fixture() -> RNStatusStats {
    RNStatusStats(
      Self.stats([
        Self.interface([
          ("name", .string("A")), ("bitrate", .int(100)), ("rxb", .int(10)), ("txb", .int(1)),
        ...
        Self.interface([
          ("name", .string("B")), ("bitrate", .int(300)), ("rxb", .int(1)), ("txb", .int(30)),
        ...
        Self.interface([
          ("name", .string("C")), ("bitrate", .int(200)), ("rxb", .int(5)), ("txb", .int(5)),
  ...
  func testSortIsStableInBothDirections() {
    ...
    let stats = RNStatusStats(value)!
    XCTAssertEqual(
      stats.sortedInterfaces(by: .rx, reverse: false).map(\.name),
      ["if0", "if1", "if2", "if3"])
    XCTAssertEqual(
      stats.sortedInterfaces(by: .rx, reverse: true).map(\.name),
      ["if0", "if1", "if2", "if3"])
  }
}
