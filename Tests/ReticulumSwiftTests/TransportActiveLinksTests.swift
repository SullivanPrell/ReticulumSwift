...
final class TransportActiveLinksTests: XCTestCase {
    ...
    func testAnEstablishedLinkIsActiveOnBothEndsAndInNeitherLinkTable() throws {
        ...
        let a = ActiveLinksLoopback(name: "a"), b = ActiveLinksLoopback(name: "b")
        a.paired = b; b.paired = a
        aT.register(interface: a)
        bT.register(interface: b)

        let aE = expectation(description: "a"), bE = expectation(description: "b")
        ...
        XCTAssertEqual(aT.activeLinks.first?.linkID, link.linkID,
