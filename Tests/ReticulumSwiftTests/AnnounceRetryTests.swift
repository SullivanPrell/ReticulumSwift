...
final class AnnounceRetryTests: XCTestCase {
  ...
  func testRetransmitsOnceAfterGrace() throws {
    ...
    XCTAssertEqual(
      outbound.sent[1].headerType, .type2, "retransmit is a HEADER_2 transport announce")
    XCTAssertEqual(
      outbound.sent[1].hops, firstHops, "retransmit carries the same hop count as the first forward"
    )
  ...
  func testPassedOnDownstreamCancelsRetry() throws {
    ...
    t.processAnnounceRetries(now: Date().timeIntervalSince1970 + 60)
    XCTAssertEqual(
      outbound.sent.count, 1, "retry cancelled after announce was passed on downstream")
  }
