...
final class UtilityErrorPageTests: XCTestCase {
  ...
  private func assertUsageIsHelpsFirstParagraph(
    _ usage: String, _ help: String,
    file: StaticString = #filePath,
    line: UInt = #line
  ) {
    XCTAssertTrue(
      usage.hasPrefix("usage: "), "usage block must start with 'usage: '",
      file: file, line: line)
    XCTAssertFalse(
      usage.contains("\n\n"), "usage block is a single paragraph",
      file: file, line: line)
    XCTAssertTrue(
      help.hasPrefix(usage), "usage block must open the help text",
      file: file, line: line)
    // The options table belongs to --help only.
    XCTAssertFalse(
      usage.contains("show this help message"),
  ...
  func testRNStatusNamesOptionsByEverySpelling() {
    ...
    XCTAssertEqual(
      parser.message(for: .missingValue("--sort")),
      "argument -s/--sort: expected one argument")
    XCTAssertEqual(
      parser.message(for: .missingValue("-w")),
      "argument -w: expected one argument")
    XCTAssertEqual(
      parser.message(for: .missingValue("--monitor-interval")),
  ...
  func testUnrecognisedArgumentsIsAlwaysPlural() {
    ...
    let parser = RNStatusApp.makeParser()
    XCTAssertEqual(
      parser.message(for: .unrecognisedOption("-Z")),
