...
final class BackboneInterfaceTests: XCTestCase {
  ...
  func testHdlcEscapingFor0x7E() {
    ...
    XCTAssertFalse(
      framed.dropFirst().dropLast().contains(0x7E),
  ...
  func testHdlcEscapingFor0x7D() {
    ...
    var found = false
    for i in 0..<(inner.count - 1) where inner[i] == 0x7D && inner[i + 1] == 0xAA {
      found = true
    }
  ...
  func testSynthesizedFromConfig() throws {
    ...
    XCTAssertEqual(
      backbone?.type, "BackboneInterface",
