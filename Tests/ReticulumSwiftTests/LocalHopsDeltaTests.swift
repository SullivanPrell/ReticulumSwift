...
final class LocalHopsDeltaTests: XCTestCase {
  ...
  func testShouldApplyDeltaFalseWhenDisabled() {
    ...
    XCTAssertFalse(
      t.shouldApplyDelta(singleDataPacket(), interface: CapturingInterface(name: "net")))
  }
  ...
  func testShouldApplyDeltaFalseWhenHopsNonZero() {
    ...
    t.localHopsDelta = 3
    XCTAssertFalse(
      t.shouldApplyDelta(singleDataPacket(hops: 1), interface: CapturingInterface(name: "net")),
  ...
  func testShouldApplyDeltaFalseForPlainDestination() {
    ...
    XCTAssertFalse(
      t.shouldApplyDelta(p, interface: CapturingInterface(name: "net")),
  ...
  func testShouldApplyDeltaFalseOnLocalClientInterface() {
    ...
    XCTAssertFalse(
      t.shouldApplyDelta(singleDataPacket(), interface: local),
  ...
  func testMangleHopsTransportInsertPromotesToHeader2() {
    let t = Transport()
    let announce = Packet(
      headerType: .type1, destinationType: .single, packetType: .announce,
    ...
    XCTAssertEqual(
      out.transportID, t.transportInstanceID, "must carry this instance's transport id")
  }
  ...
  func testRelayHopsObfuscatesLocalClientTrafficLeavingDomain() {
    ...
    XCTAssertEqual(
      t.relayHops(singleDataPacket(hops: 2), from: serving, staysLocal: false), 6,
