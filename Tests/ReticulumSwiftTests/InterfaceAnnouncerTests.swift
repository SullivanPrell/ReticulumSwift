...
final class InterfaceAnnouncerTests: XCTestCase {
  ...
  private func discoverableServer(
    name: String = "hub",
  ...
  private func fields(
    ...
    for (k, v) in entries
    where {
      guard case .uint = k else { return false }
      return true
    }() {
  ...
  func testTheDestinationMatchesTheReferenceAspects() throws {
    let a = try announcer()
    XCTAssertTrue(
      a.discoveryDestination.getName().hasPrefix("rnstransport.discovery.interface"),
  ...
  func testTheNetworkIdentityOwnsTheDestinationWhenConfigured() throws {
    ...
    let a = try announcer()
    XCTAssertTrue(
      a.discoveryDestination.getName().hasSuffix(network.hash.hexString),
  ...
  func testTheAnnounceNamesTheImplementationAndItsVersion() throws {
    ...
    XCTAssertEqual(
      f[0xFE], .bytes(try XCTUnwrap(transport.transportIdentity).hash),
  ...
  func testPublishIfacCarriesTheSegmentNameAndKey() throws {
    ...
    Reticulum.applyIfacConfiguration(
      to: server,
      from: .init(
        name: "hub", type: "TCPServerInterface",
        enabled: true,
        parameters: [
    ...
    XCTAssertNil(
      try fields(try XCTUnwrap(try announcer().announceData(for: server)))[0x07],
  ...
  func testEncryptionSetsTheFlagAndSealsThePayload() throws {
    ...
    let data = try XCTUnwrap(try announcer().announceData(for: server))
    XCTAssertEqual(
      data[data.startIndex] & InterfaceAnnounceHandler.flagEncrypted,
  ...
  func testTheJobAnnouncesTheMostOverdueInterface() throws {
    ...
    XCTAssertGreaterThan(
      overdue.lastDiscoveryAnnounce, now,
      "the most overdue interface announces")
    XCTAssertLessThan(
      recent.lastDiscoveryAnnounce, now,
  ...
  func testEnablingDiscoveryIsIdempotent() throws {
    ...
    transport.enableDiscovery(stampGenerator: generator)
    XCTAssertTrue(
      transport.interfaceAnnouncer === first,
