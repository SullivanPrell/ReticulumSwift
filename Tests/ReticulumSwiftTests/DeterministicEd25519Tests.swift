...
final class DeterministicEd25519Tests: XCTestCase {
    ...
    let v1Seed = "9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae3d55"
    let v1Pub  = "700e2ce7c4b674427eab27ba820bcf6f0faebe68e09fe8564292114e41dc6a41"
    let v1Sig  = "37b4bd5f28b61f55dc9673ae2895baceb863d9cf51780d040f98ad8cdc896cf5" +
                  "be46be655a863525da0959f7f373611585e437e28ec971b7bd206ff9bd26e803"

    // Vector 2: 1-byte message 0x72
    let v2Seed = "4ccd089b28ff96da9db6c346ec114e0f5b8a319f35aba624da8cf6ed4d0bd6d5"
    let v2Pub  = "35bf72bf49eecfbf8626be181a036058874f379675cddc176733246c142372de"
    let v2Sig  = "7d626d14eda497a22a99bd27e0477d9178f7ea4695a93f45077a170690a0eecd" +
                  "c92c27503ed403abd4c477254386466ed221c815659a868de1d24cdd618d0c0a"

    // Vector 3: 2-byte message 0xaf82
    let v3Seed = "c5aa8df43f9f837bedb7442f31dcb7b166d38535076f094b85ce3a2e0b4458f7"
    let v3Pub  = "fc51cd8e6218a1a38da47ed00230f0580816ed13ba3303ac5deb911548908025"
    let v3Sig  = "6291d657deec24024827e69c3abe01a30ce548a284743a445e3680d7db5ac3ac" +
                  "18ff9b538d16f290ae67f760984dc6594a7c15e9716ed28dc027beceea1ec40a"

    // MARK: - Public key tests

    func testVector1_publicKey() {
        let pub = DeterministicEd25519.publicKey(forSeed: hex(v1Seed))
        XCTAssertEqual(pub, hex(v1Pub), "Vector 1 public key mismatch")
    }

    func testVector2_publicKey() {
        let pub = DeterministicEd25519.publicKey(forSeed: hex(v2Seed))
        XCTAssertEqual(pub, hex(v2Pub), "Vector 2 public key mismatch")
    }

    func testVector3_publicKey() {
        let pub = DeterministicEd25519.publicKey(forSeed: hex(v3Seed))
        XCTAssertEqual(pub, hex(v3Pub), "Vector 3 public key mismatch")
    }

    // MARK: - Signature tests

    func testVector1_signature() {
        let sig = DeterministicEd25519.sign(Data(), seed: hex(v1Seed))
        XCTAssertEqual(sig, hex(v1Sig), "Vector 1 signature mismatch")
    }

    func testVector2_signature() {
        let sig = DeterministicEd25519.sign(hex("72"), seed: hex(v2Seed))
        XCTAssertEqual(sig, hex(v2Sig), "Vector 2 signature mismatch")
    }

    func testVector3_signature() {
        let sig = DeterministicEd25519.sign(hex("af82"), seed: hex(v3Seed))
        XCTAssertEqual(sig, hex(v3Sig), "Vector 3 signature mismatch")
    }
