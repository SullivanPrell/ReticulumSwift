...
final class InterfaceSpeedTrackingTests: XCTestCase {
  ...
  func testSpeedComputedFromByteDelta() {
    ...
    XCTAssertEqual(
      t.currentRxSpeed(for: iface), 8000.0, accuracy: 1.0,
      "RX speed should be 8000 bits/s (1000 bytes × 8 / 1s)")
    // TX: 500 bytes in 1 second = 4000 bits/s
    XCTAssertEqual(
      t.currentTxSpeed(for: iface), 4000.0, accuracy: 1.0,
  ...
  func testAggregateSpeedSumsInterfaces() {
    ...
    let a = CountingIface("a")
    let b = CountingIface("b")
  ...
  func testInterfaceStatsIncludesCurrentSpeed() {
    ...
    XCTAssertEqual(
      stats!.currentRxSpeed, 6400.0, accuracy: 1.0,
