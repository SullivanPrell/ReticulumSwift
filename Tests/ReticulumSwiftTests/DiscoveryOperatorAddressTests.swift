...
final class DiscoveryOperatorAddressTests: XCTestCase {
  ...
  /// test is exercising.
  private func backbone(extra: [(MsgPack.Value, MsgPack.Value)] = []) -> [(
  ...
  func testTheThreeNewFieldKeysMatchPython() {
    XCTAssertEqual(
      DiscoveryFieldKey.transportImpl.rawValue, 0xFD,
      "TRANSPORT_IMPL = 0xFD (Discovery.py:21)")
    XCTAssertEqual(
      DiscoveryFieldKey.transportVers.rawValue, 0xFC,
      "TRANSPORT_VERS = 0xFC (Discovery.py:22)")
    XCTAssertEqual(
      DiscoveryFieldKey.operatorAddress.rawValue, 0xF0,
  ...
  func testThisPortHasItsOwnImplementationIdentity() {
    XCTAssertEqual(
      InterfaceDiscoveryHelpers.implementationName, "RNSwift",
      "the identifier is this port's own, not Python's \"RNS\"")
    XCTAssertEqual(
      InterfaceDiscoveryHelpers.implementationVersion, Reticulum.version,
      """
      ...
      """)
    XCTAssertNotEqual(
      InterfaceDiscoveryHelpers.implementationName, "RNS",
  ...
  func testAnExplicitlyNilOperatorAddressIsAccepted() throws {
    ...
    let info = try XCTUnwrap(
      decode(backbone(extra: [(.uint(0xF0), .nil)])),
  ...
  func testAWrongLengthOperatorAddressIsIgnoredButTheAnnounceSurvives() throws {
    ...
    for wrong in [Data(repeating: 0x0F, count: 15), Data(repeating: 0x0F, count: 17), Data()] {
      let info = try XCTUnwrap(
        decode(backbone(extra: [(.uint(0xF0), .bytes(wrong))])),
  ...
  func testANonBytesOperatorAddressRejectsTheAnnounce() {
    ...
    // wrong-length one.
    XCTAssertNil(
      decode(backbone(extra: [(.uint(0xF0), .string("deadbeef"))])),
