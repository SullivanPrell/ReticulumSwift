...
final class InstanceConnectionTests: XCTestCase {
  ...
  private func makeConfigDirectory(shareInstance: Bool, sharedPort: UInt16, controlPort: UInt16)
    throws -> URL
  {
  ...
  func testAttach_shareInstanceDisabled_isStandalone() throws {
    ...
    let directory = try makeConfigDirectory(
      shareInstance: false, sharedPort: shared, controlPort: control)
  ...
  func testAttach_localClient_marksTransportAsBehindASharedInstance() throws {
    ...
    XCTAssertFalse(
      instance.reticulum.transport.isConnectedToSharedInstance,
