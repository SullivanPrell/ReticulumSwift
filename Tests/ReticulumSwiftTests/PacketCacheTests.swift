...
final class PacketCacheTests: XCTestCase {
  ...
  func testCacheCreatesAnnouncesSubdirectory() throws {
    ...
    XCTAssertTrue(
      FileManager.default.fileExists(atPath: announcesDir.path),
  ...
  func testCleanAnnounceCacheRemovesUnreferencedEntries() throws {
    ...
    let id1 = Identity()
    let id2 = Identity()
    let dest1 = try Destination(
      identity: id1, direction: .in, kind: .single,
      appName: "cache", aspects: ["keep"])
    let dest2 = try Destination(
      identity: id2, direction: .in, kind: .single,
    ...
    XCTAssertNotNil(
      try t.getCachedAnnounce(hash: hash1),
      "announce referenced by an active path must be retained")
    XCTAssertNil(
      try t.getCachedAnnounce(hash: hash2),
