...
final class PacketHashlistParityTests: XCTestCase {
  ...
  func testFileIsRawConcatenatedHashes() throws {
    ...
    XCTAssertEqual(
      raw.count, hashes.count * Constants.fullHashLength,
      """
  ...
  func testTrailingPartialRecordIsConsumed() throws {
    ...
    for hash in whole {
      XCTAssertTrue(
        transport.testContainsPacketHash(hash),
        "every complete record before the tear must load")
    }
    XCTAssertFalse(
      transport.testContainsPacketHash(Data(repeating: 0xEE, count: 7)),
  ...
  func testRoundTrip() throws {
    ...
    for hash in hashes {
      XCTAssertTrue(
        revived.testContainsPacketHash(hash),
