...
final class LinkEstablishmentRateTests: XCTestCase {
  ...
  func makeEstablishedLink() throws -> (initiator: Link, responder: Link) {
    ...
    let a = Loopback(name: "a")
    let b = Loopback(name: "b")
    a.paired = b
    b.paired = a
  ...
  func testEstablishmentCostMatchesPythonFormula() throws {
    ...
    // = 64*2 + 64 + 32 + 64 = 288
    let expected =
      Constants.keySize * 2 + Constants.keySize + Constants.halfKeySize + Constants.keySize
  ...
  func testDeregisterDestinationMakesItIgnoreLinkRequests() throws {
    ...
    let a = Loopback(name: "a2")
    let b = Loopback(name: "b2")
    a.paired = b
    b.paired = a
    aT.register(interface: a)
    bT.register(interface: b)
