...
final class I2PDaemonPhaseTests: XCTestCase {
  ...
  func testIdlePhaseAllowsStart() {
    XCTAssertNoThrow(
      try I2PDaemonPhase.idle.validateStart(),
      "A fresh process may start i2pd")
  }

  func testRunningPhaseRefusesASecondDaemon() {
    XCTAssertThrowsError(
      try I2PDaemonPhase.running.validateStart(),
      "i2pd's router is process-global; a second C_InitI2P would reconfigure the running one"
    ) { error in
  ...
  func testTerminatedPhaseRefusesReinitialisation() {
    XCTAssertThrowsError(
      try I2PDaemonPhase.terminated.validateStart(),
      "C_TerminateI2P leaves i2pd's globals unusable; re-initialising them is undefined behaviour"
    ) { error in
      guard case I2PDaemonError.startFailed(let message) = error else {
        return XCTFail("Expected I2PDaemonError.startFailed, got \(error)")
      }
