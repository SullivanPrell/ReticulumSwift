...
final class RNCopyMetadataTests: XCTestCase {
  ...
  func testEncodeMatchesUmsgpackGoldenBytes() {
    ...
    XCTAssertEqual(
      RNCopyApp.encodeMetadata(name: "test.txt"),
  ...
  func testDecodeRejectsGarbage() {
    ...
    XCTAssertNil(
      RNCopyApp.decodeMetadataName(MsgPack.encode(.map([(.string("other"), .bytes(Data()))]))))
  }
}
...
final class RNCopyPathHashTests: XCTestCase {
  ...
  func testSpinnerAndEraseConstants() {
    ...
    XCTAssertEqual(
      String(RNCopyApp.spinnerFrames), "\u{2884}\u{2882}\u{2881}\u{2841}\u{2848}\u{2850}\u{2860}")
...
final class RNCopyAllowedIdentitiesTests: XCTestCase {
  ...
  func testDecodeRejectsNonHex() {
    XCTAssertThrowsError(try RNCopyApp.decodeAllowedIdentity(String(repeating: "z", count: 32))) {
      error in
      guard let error = error as? RNCopyApp.AllowedIdentityError else {
        return XCTFail("wrong error type")
      }
  ...
  func testDecodeAccepts32Hex() throws {
    XCTAssertEqual(
      try RNCopyApp.decodeAllowedIdentity("00112233445566778899aabbccddeeff").count, 16)
  }
  ...
  func testSearchOrderIsEtcThenXdgThenDotDir() {
    ...
    XCTAssertEqual(
      RNCopyApp.allowedIdentitiesSearchPaths(home: "/h"),
      [
...
final class RNCopyFetchJailTests: XCTestCase {
  ...
  func testNoJailResolvesAbsolute() {
    ...
    XCTAssertEqual(
      RNCopyApp.resolveFetchPath(requested: "~/a.txt", jail: nil, fileSystem: fs),
      .serve(path: "/h/a.txt"))
  }
  ...
  func testNoJailAllowsAnyReadablePath() {
    ...
    XCTAssertEqual(
      RNCopyApp.resolveFetchPath(requested: "/etc/passwd", jail: nil, fileSystem: fs),
  ...
  func testJailStripsPrefixOccurrences() {
    ...
    XCTAssertEqual(
      RNCopyApp.resolveFetchPath(requested: "/j/sub/x", jail: "/j", fileSystem: fs),
      .serve(path: "/j/sub/x"))
  }
  ...
  func testJailEscapeReturnsNotAllowed() {
    ...
    XCTAssertEqual(
      RNCopyApp.resolveFetchPath(requested: "../etc/passwd", jail: "/j", fileSystem: fs),
  ...
  func testJailedMissingFileReturnsNotFound() {
    let fs = MockRNCopyFileSystem()
    XCTAssertEqual(
      RNCopyApp.resolveFetchPath(requested: "a.txt", jail: "/j", fileSystem: fs),
  ...
  func testReplaceAllSemantics() {
    ...
    XCTAssertEqual(
      RNCopyApp.resolveFetchPath(requested: "/j//j/x", jail: "/j", fileSystem: fs),
      .serve(path: "/j/x"))
  }
}
...
final class RNCopySaveTargetTests: XCTestCase {
  ...
  func testNoSavePathIsRelative() {
    ...
    let fs = MockRNCopyFileSystem()
    XCTAssertEqual(
      RNCopyApp.resolveSaveTarget(
        filename: "a.txt", savePath: nil,
...
final class RNCopyIdentityTests: XCTestCase {
  ...
  func testDefaultIdentityPath() {
    ...
    XCTAssertEqual(
      RNCopyApp.defaultIdentityPath(storagePath: URL(fileURLWithPath: "/s")).path,
      "/s/identities/rncp")
  }
