...
final class TCPInterfaceNamingTests: XCTestCase {
  ...
  func testConfiguredClientIsVisibleInRnstatus() {
    ...
    XCTAssertTrue(
      rendered.contains("TCPInterface[wisco.network TCP/147.224.161.226:4242]"),
  ...
  func testSpawnedServerClientIsHiddenFromRnstatus() {
    ...
    XCTAssertFalse(
      rendered.contains("TCPInterface[Client"),
      "spawned server clients must stay hidden by default:\n\(rendered)")
    XCTAssertTrue(
      renderRegistered([client], showAll: true).contains(
        "TCPInterface[Client on TCP Server/10.0.0.9:51420]"))
  }
  ...
  func testSynthesisNamesEachClientAfterItsConfigBlock() throws {
    ...
    XCTAssertEqual(
      names,
      [
        "TCPInterface[Backbone A/10.0.0.1:4242]",
  ...
  func testUDPInterfaceReportsItsConfiguredBindAddress() {
    ...
    XCTAssertEqual(
      UDPInterface(name: "any", listenPort: 4242).displayName,
  ...
  func testSynthesisReadsListenIPForTheServerName() throws {
    ...
    XCTAssertTrue(
      names.contains("TCPServerInterface[TCP Server/127.0.0.1:4242]"),
      "got \(names)")
  }
}
