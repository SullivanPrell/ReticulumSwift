...
final class ResourceSafeguardsTests: XCTestCase {
  ...
  private var aT: Transport!
  private var bT: Transport!
  private var aI: RecordingLoopback!
  private var bI: RecordingLoopback!
  ...
  private func establishLink() throws -> (Link, Link) {
    ...
    aI = RecordingLoopback(name: "A")
    bI = RecordingLoopback(name: "B")
    aI.paired = bI
    bI.paired = aI
  ...
  func testReceiverCancelEmitsResourceReceiverCancel() throws {
    ...
    XCTAssertTrue(
      bI.sentContexts.contains(.resourceReceiverCancel),
