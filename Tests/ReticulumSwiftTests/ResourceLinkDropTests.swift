//===----------------------------------------------------------------------===//
// Copyright (c) 2026 ReticulumSwift contributors.
//
// Licensed under the Reticulum License. See LICENSE in the repository root for
// the full license text, and NOTICE for attribution of the upstream project
// this file is derived from.
//
// SPDX-License-Identifier: LicenseRef-Reticulum
//===----------------------------------------------------------------------===//

import XCTest

@testable import ReticulumSwift

/// Tests for Resource behavior when the link drops during transfer.
final class ResourceLinkDropTests: XCTestCase {

  var aT: Transport!
  var bT: Transport!

  final class LoopbackInterface: Interface {
    var name: String
    var bitrate: Int = 0
    var isOnline: Bool = true
    weak var paired: LoopbackInterface?
    var inboundHandler: ((Packet, any Interface) -> Void)?
    var dropPackets = false  // Set to drop all further packets
    init(name: String) { self.name = name }
    func start() throws { isOnline = true }
    func stop() { isOnline = false }
    func send(_ packet: Packet) throws {
      guard !dropPackets else { return }
      let raw = try packet.pack()
      let copy = try Packet.unpack(raw)
      paired?.inboundHandler?(copy, paired!)
    }
  }

  private func establishLink() throws -> (Link, Link, LoopbackInterface, LoopbackInterface) {
    aT = Transport()
    bT = Transport()
    let bId = Identity()
    let bDest = try Destination(
      identity: bId, direction: .in, kind: .single,
      appName: "test", aspects: ["drop"])
    bT.ownerIdentity = bId
    bT.register(destination: bDest)
    let aI = LoopbackInterface(name: "A")
    let bI = LoopbackInterface(name: "B")
    aI.paired = bI
    bI.paired = aI
    aT.register(interface: aI)
    bT.register(interface: bI)
    let aE = expectation(description: "a")
    let bE = expectation(description: "b")
    aT.onLinkEstablished = { _ in aE.fulfill() }
    bT.onLinkEstablished = { _ in bE.fulfill() }
    let aLink = try Link.initiate(destination: bDest, transport: aT)
    wait(for: [aE, bE], timeout: 1.0)
    let bLink = try XCTUnwrap(bT.links[aLink.linkID!])
    return (aLink, bLink, aI, bI)
  }

  func testResourceCancelledWhenLinkDrops() throws {
    let (aLink, bLink, _, _) = try establishLink()
    let payload = Data(repeating: 0xAA, count: 100)

    let failed = expectation(description: "failed")
    let sender = ResourceTransfer(link: aLink)
    sender.onFailed = { _, _ in failed.fulfill() }

    let receiver = ResourceTransfer(link: bLink)
    receiver.bindAsReceiver()

    try sender.send(payload: payload)

    // Cancel the sender explicitly
    sender.cancel()

    wait(for: [failed], timeout: 1.0)
    XCTAssertTrue(sender.status.isTerminal)
  }

  func testResourceStatusIsCorrectAfterCancel() throws {
    let (aLink, _, _, _) = try establishLink()
    let sender = ResourceTransfer(link: aLink)
    try sender.send(payload: Data(repeating: 0xBB, count: 100))
    sender.cancel()

    if case .failed = sender.status {
    } else {
      XCTFail("Expected .failed status after cancel")
    }
  }
}
