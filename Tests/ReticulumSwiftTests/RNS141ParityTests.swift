...
final class RNS141ParityTests: XCTestCase {
  ...
  private final class StubInterface: Interface {
    ...
    init(
      name: String, mode: InterfaceMode = .full, gravity: Int = 0,
      createdAt: Date = Date()
    ) {
      self.name = name
      self.mode = mode
  ...
  func testInterfaceGravityDefaultsToZero() {
    XCTAssertEqual(
      InterfaceMode.defaultGravity, 0,
  ...
  func testBoundarySearchModesMatchesPython() {
    XCTAssertEqual(
      InterfaceMode.boundarySearchModes, [.boundary, .gateway],
  ...
  func testPerInterfaceGravityOverridesDefault() {
    ...
    XCTAssertNil(
      cfg.interfaces.first { $0.name == "Ordinary" }?.int("gravity"),
  ...
  func testAutoconnectInterfaceModeAliases() {
    ...
    for (name, mode) in expected {
      ...
      XCTAssertEqual(
        InterfaceMode(configName: name.uppercased()), mode,
        "alias \(name) must be case-insensitive")
    }
    XCTAssertNil(
      InterfaceMode(configName: "not-a-mode"),
  ...
  func testHigherGravityInterfaceTakesOverPath() throws {
    ...
    XCTAssertEqual(
      t.paths[dest.hash]?.nextHopInterfaceName, "low",
    ...
    XCTAssertEqual(
      t.paths[dest.hash]?.nextHopInterfaceName, "high",
  ...
  func testTCPServerSpawnedClientInheritsGravity() {
    ...
    let client = TCPServerClientInterface(
      name: "Client on srv", parentServer: server,
  ...
  func testEgressLimitRequiresBurstMinSamples() {
    ...
    XCTAssertEqual(
      InterfaceFreqTracker.minSamples, 2,
  ...
  func testIngressBurstClearsOnADequeTooSmallForTheEgressFloor() {
    ...
    XCTAssertTrue(
      t.shouldIngressLimit(on: iface, now: t0 + 0.03), "the flood must activate a burst")
  ...
  func testMismatchedProofIsNotAcceptedWhenRebalanceDisabled() throws {
    ...
    let aI = LoopInterface(name: "A")
    let bI = LoopInterface(name: "B")
    aI.paired = bI
    bI.paired = aI
  ...
  func testSetMaxRequestSizeRejectsNegative() throws {
    ...
    XCTAssertThrowsError(
      try dest.setMaxRequestSize(-1),
      "Python raises ValueError for a negative maximum"
    ) { error in
  ...
  func testRequestReceiptMaxResponseSizeDefaultsToNil() {
    let r = RequestReceipt(
      requestID: Data(repeating: 1, count: 16),
  ...
  func testExtremeLogLevelIsEight() {
    XCTAssertEqual(
      Reticulum.LogLevel.extreme.rawValue, 8,
  ...
  func testChannelRejectsFarFutureSequence() throws {
    ...
    XCTAssertEqual(
      delivered, Channel.windowMaxLimit + 1,
