...
final class StorageStartupTests: XCTestCase {
  ...
  func testPreParityFilesAreLeftUntouched() throws {
    ...
    XCTAssertEqual(
      Set(legacy.keys), Set(StorageInventory.preParityOrphans),
    ...
    XCTAssertTrue(
      ...
      """)
    XCTAssertNil(
      rns.transport.recall(identity: Data(hex: "aabbccddeeff00112233445566778899")!),
      "known destinations start empty for the same reason")
    XCTAssertFalse(
      rns.transport.testContainsPacketHash(Data(hex: "aabb")!),
    ...
    for (name, contents) in legacy {
      XCTAssertEqual(
        try Data(contentsOf: dir.appendingPathComponent(name)), contents,
        """
