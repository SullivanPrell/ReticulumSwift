...
final class ReticulumLifecycleTests: XCTestCase {
  ...
  func testPathTablePersistedAndRestored() throws {
    ...
    do {
      ...
      destHash = try installPersistablePath(
        on: rns.transport, through: iface,
        hops: 2, aspect: "lifecycle"
      ).destinationHash
      rns.stop()
    }
  ...
  func testTransportAccessibleFromShared() throws {
    ...
    XCTAssertFalse(
      Reticulum.shouldUseImplicitProof() == false, "should use implicit proof by default")
    rns.stop()
  }
}
