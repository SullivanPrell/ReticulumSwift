...
final class RSGTests: XCTestCase {
  ...
  func testMinimalEnvelopeGoldenBytes() {
    let signedData = RSG.SignedData(
      ...
      meta: [
        ("signer", .bytes(Data(repeating: 0x02, count: 16))),
        ("pubkey", .bytes(Data(repeating: 0x03, count: 64))),
      ],
      message: nil)

    let expected =
      "83a86861736874797065a6736861323536a468617368c420"
    ...
    XCTAssertEqual(
      RNIDEncoding.hexEncode(envelope.prefix(20)),
  ...
  func testCreateProducesA224ByteMinimalRSG() throws {
    ...
    guard
      case .binary(let rsg) = try RSG.create(signer: identity, message: .bytes(Data("hello".utf8)))
  ...
  func testCreateThrowsForAPublicOnlyIdentity() throws {
    ...
    XCTAssertThrowsError(try RSG.create(signer: publicOnly, message: .bytes(Data("x".utf8)))) {
      error in
  ...
  func testCreateRejectsAnUnknownOutputFormatName() {
    ...
    XCTAssertThrowsError(
      try RSG.create(
        signer: identity, message: .bytes(Data("x".utf8)),
        outputName: "rot13")
    ) { error in
  ...
  func testCreateSilentlyIgnoresEmptyMeta() throws {
    ...
    guard
      case .binary(let rsg) = try RSG.create(
        signer: identity, message: .bytes(Data("x".utf8)),
  ...
  private func makeRSG(
    signer: Identity,
    message: Data,
    mutate: (inout [(String, MsgPack.Value)]) -> Void = { _ in }
  ) throws -> Data {
    var entries: [(String, MsgPack.Value)] = [
      ...
      (
        "meta",
        .map([
          (.string("signer"), .bytes(signer.hash)),
  ...
  func testValidateRejectsAnUndecodableEnvelope() throws {
    ...
    let result = try RSG.validate(
      rsgData: rsg, message: .bytes(Data("x".utf8)), requiredSigner: .none)
  ...
  func testValidateSignerMismatchReturnsSigningIdentityButNoSignedData() throws {
    ...
    let result = try RSG.validate(
      rsgData: rsg, message: .bytes(message),
  ...
  func testValidateHashMismatchReturnsSigningIdentityButNoSignedData() throws {
    ...
    let result = try RSG.validate(
      rsgData: rsg, message: .bytes(Data("different".utf8)),
      requiredSigner: .none)
    XCTAssertFalse(result.isValid)
    XCTAssertNil(result.signedData)
    XCTAssertEqual(result.signingIdentity?.hash, identity.hash)
  ...
  func testValidateThrowsForAnEmptyMessage() throws {
    ...
    XCTAssertThrowsError(
      try RSG.validate(
        rsgData: rsg, message: .bytes(Data()),
        requiredSigner: .none)
    ) { error in
  ...
  func testValidateRejectsA63ByteInput() throws {
    let result = try RSG.validate(
      rsgData: Data(count: 63), message: .bytes(Data("x".utf8)),
      requiredSigner: .none)
    XCTAssertFalse(result.isValid)
    XCTAssertNil(result.signedData)
  }
  ...
  func testRequiredSignerIdentityIgnoresTheEmbeddedPubkey() throws {
    ...
    let withSigner = try RSG.validate(
      rsgData: rsg, message: .bytes(message),
    ...
    let withoutSigner = try RSG.validate(
      rsgData: rsg, message: .bytes(message),
      requiredSigner: .none)
  ...
  func testArmourPadsTheFinalShortRow() {
    ...
    XCTAssertEqual(
      String(lines[2]), String(repeating: "a", count: 36) + String(repeating: "=", count: 28))
  ...
  func testUnwrapRoundTripsWrap() {
    for length in [1, 63, 64, 65, 128] {
      ...
      XCTAssertEqual(
        unwrapped?.replacingOccurrences(of: "=", with: ""), payload,
        "length \(length)")
    }
  }
  ...
  func testCheckReleaseRSMStructure() {
    ...
    let shortOrigin: [(String, MsgPack.Value)] = [
      ("name", .string("pkg")),
      ("version", .string("1.0")),
    ...
    let stringOrigin: [(String, MsgPack.Value)] = [
      ("name", .string("pkg")),
      ("version", .string("1.0")),
    ...
    let valid: [(String, MsgPack.Value)] = [
      ("name", .string("pkg")),
      ("version", .string("1.0")),
      ("origin", .bytes(Data(count: 16))),
      ("path", .string("/x")),
