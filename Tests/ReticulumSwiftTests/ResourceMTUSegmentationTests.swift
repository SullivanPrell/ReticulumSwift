...
final class ResourceMTUSegmentationTests: XCTestCase {
  ...
  func testPartSizeFollowsTheNegotiatedMTURatherThanTheBaseConstant() throws {
    ...
    XCTAssertEqual(
      Resource.segmentSize(for: link), referenceSDU(mtu: 8156),
      "part size must come from this link, not from the 500-byte base constant")
    XCTAssertNotEqual(
      Resource.segmentSize(for: link), Constants.mdu,
  ...
  func testReceiverDerivesThePartCountAndSurfacesADisagreement() throws {
    ...
    XCTAssertEqual(
      derived, Int((Double(size) / Double(sdu)).rounded(.up)),
    ...
    XCTAssertNotEqual(
      derived, advertisedByAStaleSender,
      "this scenario is only meaningful if the two disagree")
    XCTAssertTrue(
      ResourceTransfer.partCountDisagrees(
        advertised: advertisedByAStaleSender,
        derived: derived),
