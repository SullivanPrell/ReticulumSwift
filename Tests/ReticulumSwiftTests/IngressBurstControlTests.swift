...
final class IngressBurstControlTests: XCTestCase {
  ...
  func testBurstDeactivatesAfterHoldPeriod() {
    ...
    XCTAssertFalse(
      t.shouldIngressLimit(on: iface, now: nowAfterHold + 0.001),
  ...
  func testPathRequestBurstDeactivationStillLimitsOnClearingCall() {
    ...
    for probe in 0..<IngressControlState.icPrBurstCooldown {
      XCTAssertTrue(
        t.shouldIngressLimitPR(on: iface, now: nowAfterHold + Double(probe)),
  ...
  func testHoldAnnounceStoresPacket() throws {
    ...
    XCTAssertEqual(
      t.heldAnnounceCount(for: iface), 1,
  ...
  func testProcessHeldAnnouncesReleasesLowestHop() throws {
    ...
    XCTAssertEqual(
      released?.destinationHash, pktLow.destinationHash,
