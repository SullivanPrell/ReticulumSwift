...
enum InterfaceConformers {
  ...
  static func everyConcreteInterface() throws -> [any Interface] {
    ...
    all.append(
      SerialInterface(
        name: "serial0", port: "/dev/null",
        transport: MockSerialPort()))
    all.append(
      KISSInterface(
        name: "kiss0", port: "/dev/null",
        transport: MockSerialPort()))
    all.append(
      try AX25KISSInterface(
        name: "ax250", port: "/dev/null",
        callsign: "NOCALL", ssid: 0,
        transport: MockSerialPort()))
    all.append(
      WeaveInterface(
        name: "weave0", port: "/dev/null",
        transport: MockSerialPort()))

    // RNode family.
    all.append(RNodeInterface(name: "rnode0", transport: MockRNodeTransport()))
    let sub = RNodeSubInterface(
      name: "sub0", index: 0, interfaceType: "LoRa",
      frequency: 867_200_000, bandwidth: 125_000,
      txPower: 0, sf: 8, cr: 5)
    all.append(sub)
    all.append(
      try RNodeMultiInterface(
        name: "rnodemulti0",
    ...
    let i2p = I2PInterface(
      name: "i2p0", daemon: MockI2PDaemon(),
    ...
    all.append(
      TCPServerClientInterface(
        name: "Client on tcpserver0",
        parentServer: server,
        peerHost: "10.0.0.9", peerPort: 51000))

    let weaveOwner = WeaveInterface(
      name: "weaveowner0", port: "/dev/null",
      transport: MockSerialPort())
    all.append(
      WeaveInterfacePeer(
        owner: weaveOwner,
