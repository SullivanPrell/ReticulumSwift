...
private func makePayload(
  info: [(MsgPack.Value, MsgPack.Value)], stamp: Data = Data(repeating: 0xAB, count: 32)
) -> Data {
...
private func backboneInfo(
  name: String = "TEST SERVER", reachableOn: String = "192.168.1.100", port: Int = 4965
...
final class InterfaceDiscoveryUtilTests: XCTestCase {
  ...
  func testSanitizeNameStripsNonAlphanumericEnds() {
    XCTAssertEqual(
      InterfaceAnnounceHandler.sanitizeName("--Copenhagen hub--"),
      "Copenhagen hub")
  }
  ...
  func testIsHostnameTooLong() {
    ...
    let long =
      ...
      + String(repeating: "c", count: 64) + "." + String(repeating: "d", count: 65) + ".com"
...
final class InterfaceAnnounceHandlerTests: XCTestCase {
  ...
  func testInvalidInterfaceTypeIgnored() {
    var called = false
    let handler = InterfaceAnnounceHandler(requiredValue: 14, stampValidator: passthrough) { _ in
      called = true
    }
  ...
  func testStampValidationFailureBlocksCallback() {
    ...
    let handler = InterfaceAnnounceHandler(
      requiredValue: 14, stampValidator: RejectStampValidator()
    ) { _ in called = true }
    let payload = makePayload(info: backboneInfo())
    handler.receivedAnnounce(
      destinationHash: Data(repeating: 0, count: 16),
  ...
  func testBackboneConfigEntryContainsHost() {
    ...
    XCTAssertTrue(
      entry.contains("BackboneInterface") || entry.contains("TCPClientInterface"),
