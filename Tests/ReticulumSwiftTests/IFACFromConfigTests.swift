...
final class IFACFromConfigTests: XCTestCase {
  ...
  private func synthesise(_ config: String) throws -> [any Interface] {
    ...
    let reticulum = Reticulum(
      configuration: .init(
        storagePath: tmp.appendingPathComponent("storage"),
        shareInstance: false))
  ...
  func testBothConfigKeySpellingsAreAccepted() throws {
    let underscored = try synthesise(
      udpBlock(
        """
            network_name = seg
            pass_phrase = secret
        """))
    let compact = try synthesise(
      udpBlock(
        """
            networkname = seg
            passphrase = secret
        """))
    XCTAssertEqual(
      try XCTUnwrap(underscored.first?.ifacKey),
  ...
  func testEmptyNetworkNameAndPassphraseLeaveIFACOff() throws {
    let interfaces = try synthesise(
      udpBlock(
        ...
        """))
    XCTAssertNil(
      try XCTUnwrap(interfaces.first).ifacKey,
  ...
  func testIFACSizeIsReadInBits() throws {
    let interfaces = try synthesise(
      udpBlock(
        ...
        """))
    XCTAssertEqual(
      try XCTUnwrap(interfaces.first).ifacSize, 8,
  ...
  func testIFACSizeBelowTheMinimumIsIgnored() throws {
    let interfaces = try synthesise(
      udpBlock(
        ...
        """))
    XCTAssertEqual(
      try XCTUnwrap(interfaces.first).ifacSize, Constants.defaultIfacSize,
  ...
  func testIFACNetnameAppearsInStatusOutput() throws {
    ...
    try reticulum.synthesizeInterfaces(
      from: ReticulumConfig.parse(
        udpBlock(
          """
              network_name = internal_1
              passphrase = secret
    ...
    XCTAssertEqual(
      row.string("ifac_netname"), "internal_1",
      """
  ...
  func testIFACNetnameIsNilWithoutASegment() throws {
    ...
    reticulum.transport.transportIdentity = Identity()
    try reticulum.synthesizeInterfaces(
      from: ReticulumConfig.parse(udpBlock("    listen_port = 4967")))
  ...
  private func pairOnSegments(
    _ left: (String, String)?,
    _ right: (String, String)?
  ) throws -> (PairedInterface, PairedInterface) {
