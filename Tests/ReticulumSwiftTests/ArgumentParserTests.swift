...
final class ArgumentParserTests: XCTestCase {
  ...
  func testUnrecognisedOptionRendersAsPlural() {
    ...
    XCTAssertEqual(
      ArgumentError.unrecognisedOption("--bogus").description,
  ...
  func testMessagePassesThroughNonOptionErrors() {
    let parser = makeParser()
    XCTAssertEqual(
      parser.message(for: .unrecognisedOption("--bogus")),
      "unrecognized arguments: --bogus")
    XCTAssertEqual(
      parser.message(for: .ambiguousOption("--ver", ["--version", "--verbose"])),
