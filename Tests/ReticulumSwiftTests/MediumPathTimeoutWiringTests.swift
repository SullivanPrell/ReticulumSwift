...
final class MediumPathTimeoutWiringTests: XCTestCase {
    ...
    func testAStandaloneProcessAnswersFromItsOwnTransport() {
        ...
        XCTAssertEqual(InstanceConnection.mediumPathTimeout(rpc: nil, transport: t),
    ...
    func testProbeTimeoutTakesTheLargerOfTheHopSumAndTheMediumTimeout() {
        ...
        // Medium timeout wins.
        XCTAssertEqual(NetworkProbe.effectiveTimeout(nil, firstHopTimeout: 6, mediumPathTimeout: 900),
                       900, accuracy: 1e-9)
    }

    func testAnExplicitProbeTimeoutOverridesBoth() {
        XCTAssertEqual(NetworkProbe.effectiveTimeout(3, firstHopTimeout: 6, mediumPathTimeout: 900),
                       3, accuracy: 1e-9)
    }
    ...
    func testZeroStillMeansUnset() {
        XCTAssertEqual(NetworkProbe.effectiveTimeout(0, firstHopTimeout: 6, mediumPathTimeout: 900),
                       900, accuracy: 1e-9)
    }
    ...
    private func pollsBeforeGivingUp(timeout: TimeInterval, mediumPathTimeout: TimeInterval) -> Int {
        ...
        let runner = RNPathRunner(options: options,
                                  ...
                                  resolver: resolver,
                                  now: { tick += 1; return Date(timeIntervalSince1970: 1_700_000_000 + tick) },
                                  sleep: { _ in },
                                  output: { _ in })
        _ = runner.run()
    ...
    func testTheSpinnerDeadlineIsFlooredAtTheMediumPathTimeout() {
        ...
        XCTAssertGreaterThan(floored, short + 40,
    ...
    func testAnExplicitTimeoutLongerThanTheFloorIsNotShortened() {
        XCTAssertEqual(pollsBeforeGivingUp(timeout: 60, mediumPathTimeout: 5),
...
private final class StubManagement: RNPathManagementSource {
    ...
    func blackholeIdentity(_ identityHash: Data, until: TimeInterval?, reason: String?) throws -> Bool? { nil }
...
private final class NeverResolves: RNPathPathResolver {
    ...
    func hasPath(to destinationHash: Data) -> Bool { hasPathCalls += 1; return false }
