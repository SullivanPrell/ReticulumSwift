...
final class RNStatus152SurfaceTests: XCTestCase {
    ...
    func testTheFourNewFlagsParse() throws {
        ...
        for (short, long) in [("-b", "--blocked-ips"), ("-p", "--pps"),
    ...
    func testUsageBlockCarriesTheNewFlags() {
        ...
        XCTAssertTrue(RNStatusApp.errorText("unrecognized arguments: -Q").hasSuffix(
    ...
    func testEverySortKeyReadsADistinctPayloadField() {
        ...
        for (sort, key) in fields {
            let stats = Self.top([Self.iface([("name", .string("low")), (key, .int(1))]),
                                  Self.iface([("name", .string("high")), (key, .int(9))])])
            XCTAssertEqual(stats.sortedInterfaces(by: sort, reverse: false).map(\.name),
                           ["high", "low"], "descending on \(sort.rawValue)")
            XCTAssertEqual(stats.sortedInterfaces(by: sort, reverse: true).map(\.name),
    ...
    func testRateCarriesTheMTU() {
        XCTAssertEqual(Self.renderer().render(stats: Self.top([Self.iface()]), linkCount: nil),
                       Self.plainInterface + "\n")
    }
    ...
    func testZeroGravityAddsNothing() {
        XCTAssertTrue(Self.renderer().render(stats: Self.top([Self.iface()]), linkCount: nil)
                        .contains("    Status    : Up\n"))
    }
    ...
    func testViolationsLine() {
        ...
        for (overrides, expected) in cases {
            let rendered = Self.renderer().render(stats: Self.top([Self.iface(overrides)]), linkCount: nil)
            if let expected {
                XCTAssertTrue(rendered.contains("    Violatns. : \(expected)\n"),
    ...
    func testFilterHitsLine() {
        ...
        XCTAssertTrue(Self.renderer().render(stats: stats, linkCount: nil)
                        .contains("    Flt. Hits : 9\n"))
        XCTAssertFalse(Self.renderer().render(stats: Self.top([Self.iface()]), linkCount: nil)
                        .contains("Flt. Hits"))
    }
    ...
    func testPacketsPerSecondInTotals() {
        let stats = Self.top([Self.iface([("rxs", .double(24000)), ("txs", .double(9600))])],
                             [("rxpps", .int(42)), ("txpps", .int(17))])
        let rendered = Self.renderer { $0.trafficTotals = true; $0.pps = true }
            .render(stats: stats, linkCount: nil)
    ...
    func testPacketsPerSecondNeedsTotals() {
        ...
        XCTAssertFalse(Self.renderer { $0.pps = true }.render(stats: stats, linkCount: nil)
                        .contains("pps"))
    }
    ...
    func testTotalsWithAnnounceAndPathRequestAggregates() {
        ...
        let rendered = Self.renderer {
            $0.trafficTotals = true; $0.announceStats = true; $0.prStats = true
        }.render(stats: stats, linkCount: nil)
        XCTAssertTrue(rendered.hasSuffix(
    ...
    func testQueuePressureOnAnIdleInstance() {
        let rendered = Self.renderer { $0.queueStats = true }
            .render(stats: Self.top([Self.iface([("rxs", .double(24000)), ("txs", .double(9600))])]),
                    linkCount: nil)
        XCTAssertTrue(rendered.hasSuffix(
    ...
    func testQueuePressureWithDepthsAndDrops() {
        ...
        XCTAssertTrue(Self.renderer { $0.queueStats = true }
    ...
    func testQueueStatsAgainstAPeerWithoutTheKeys() {
        ...
        XCTAssertTrue(Self.renderer { $0.queueStats = true }.render(stats: bare, linkCount: nil)
    ...
    func testDiscoveredDetailsRenderTheOperatorLXMFAddress() throws {
        var info = DiscoveredInterfaceInfo(type: "TCPServerInterface", transport: true,
