...
private final class MockWeaveTransport: SerialPortTransport {
  ...
  func open(
    port: String, baudRate: Int,
    dataBits: Int, parity: SerialParity, stopBits: Int
  ) throws {
    openCalled = true
    isOpen = true
...
final class WeaveInterfaceTests: XCTestCase {
  ...
  func testWeaveLogFrameInit() {
    let frame = WeaveLogFrame(
      timestamp: 1.5, level: 3, event: 0x3002,
      data: Data([0x01, 0x02]))
  ...
  func testPeerJobsPrunesStaleDeviceEndpoints() throws {
    let iface = WeaveInterface(
      name: "W0", port: "/dev/null",
  ...
  func testIncomingFrameEndpointPkt() throws {
    ...
    let parent = WeaveInterface(
      name: "test-weave", port: "/dev/null",
      transport: MockWeaveTransport())
  ...
  func testDedupDequeMaxLength() throws {
    ...
    for i in 0..<(WeaveInterface.multiIfDequeLen + 2) {
      peer.processIncoming(
        data: Data([UInt8(i & 0xFF), UInt8(i >> 8)]),
        endpointAddr: epID)
    }
  ...
  func testWeaveInterfaceConformsToInterface() throws {
    let iface: any Interface = WeaveInterface(
      name: "W0", port: "/dev/null",
      transport: MockWeaveTransport())
    XCTAssertFalse(iface.isOnline)
    XCTAssertEqual(iface.bitrate, WeaveInterface.bitrateGuess)
