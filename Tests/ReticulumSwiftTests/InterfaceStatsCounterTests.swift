...
final class InterfaceStatsCounterTests: XCTestCase {
  ...
  func testAMalformedFrameIsCounted() throws {
    ...
    XCTAssertEqual(
      try statsForFirstInterface(t)["protocol_violations"]?.asInt, 0,
    ...
    XCTAssertEqual(
      try statsForFirstInterface(t)["protocol_violations"]?.asInt, 1,
  ...
  func testViolationsAccumulateRatherThanLatch() throws {
    ...
    XCTAssertEqual(
      try statsForFirstInterface(t)["protocol_violations"]?.asInt, 3,
  ...
  func testADuplicatePacketCountsAsAPacketFilterHit() throws {
    ...
    XCTAssertEqual(
      t.interfaceCounts(for: iface).packetFilterHits, 0,
      "the first sighting passes the filter, so nothing is filtered yet")

    t.handleIncoming(packet: packet, from: iface)
    XCTAssertEqual(
      t.interfaceCounts(for: iface).packetFilterHits, 1,
      """
