...
final class DaemonBootstrapTests: XCTestCase {
  ...
  func testResolveConfigDirDelegatesToInstanceConnection() throws {
    ...
    for explicit in [nil, "/tmp/explicit"] as [String?] {
      let viaBootstrap = DaemonBootstrap.resolveConfigDir(
        explicit: explicit,
        home: temporaryDirectory,
        systemConfigDir: etc)
      let viaConnection = InstanceConnection.resolveConfigDirectory(
        ...
        fileManager: .default)
      XCTAssertEqual(
        viaBootstrap.path, viaConnection.path, "explicit: \(String(describing: explicit))")
    }
  }
  ...
  func testCreateStorageTree() throws {
    ...
    for directory in [
      paths.storage, paths.cache, paths.announceCache, paths.resources,
  ...
  func testRequestedLogLevelWinsAndClampsAtEight() {
    ...
    XCTAssertEqual(
      DaemonBootstrap.effectiveLogLevel(
        configLogLevel: 4,
  ...
  func testConfiguredLogLevelReadsLoggingSection() {
    ...
    XCTAssertNil(
      DaemonBootstrap.configuredLogLevel(inConfigText: "[logging]\nlogtimestamps = no\n"))
  ...
  func testConfiguredLogLevelOfShippedTemplates() {
    ...
    XCTAssertEqual(
      DaemonBootstrap.configuredLogLevel(inConfigText: RNSConfigTemplates.defaultConfig), 4)
    XCTAssertEqual(
      DaemonBootstrap.configuredLogLevel(inConfigText: RNSConfigTemplates.exampleConfig), 4)
  }
