...
final class InterfaceConstructionTests: XCTestCase {
  ...
  private final class StubSerialPort: SerialPortTransport {
    ...
    var isOpen = false
    func open(
      port: String, baudRate: Int, dataBits: Int,
      parity: SerialParity, stopBits: Int
    ) throws { isOpen = true }
  ...
  func testEveryPreviouslyUnconstructibleTypeIsConstructedFromItsConfigBlock() throws {
    ...
    let rnode = try XCTUnwrap(
      byName["LoRa"] as? RNodeInterface,
    ...
    XCTAssertEqual(
      i2p.peers, ["base32one.b32.i2p", "base32two.b32.i2p"],
  ...
  func testRNodeStationIdentificationConfigures() throws {
    registerStubs()
    let stack = makeStack()
    try stack.synthesizeInterfaces(
      from: parse(
        """
          [[LoRa]]
            type = RNodeInterface
  ...
  func testUnknownTypeIsSkippedLoudlyWhileOthersConstruct() throws {
    ...
    try stack.synthesizeInterfaces(
      from: parse(
        ...
        """))
    XCTAssertNil(
      stack.transport.interfaces.first { $0.name == "Mystery" },
      "an unknown type must not produce an interface")
    XCTAssertNotNil(
      stack.transport.interfaces.first { $0.name == "Real" },
  ...
  func testAnOutOfRangeRadioParameterFailsConstruction() {
    ...
    XCTAssertThrowsError(
      ...
      XCTAssertEqual(
        error as? InterfaceConstructionError,
  ...
  func testAnUnregisteredFamilyFailsWithTheCause() {
    ...
    XCTAssertThrowsError(
      ...
      guard
        case .unavailable(let family, let device, _)? =
          error as? InterfaceTransportFactories.FactoryError
      else {
        return XCTFail("expected FactoryError.unavailable, got \(error)")
      }
      XCTAssertEqual(family, "serial")
      XCTAssertEqual(
        device, "/dev/cu.usbserial-kiss",
  ...
  func testBLEDeviceStringWithoutAnAppFactoryNamesTheRegistration() {
    ...
    XCTAssertThrowsError(
      ...
      guard
        case .unavailable(_, let device, let hint)? =
          error as? InterfaceTransportFactories.FactoryError
      else {
        return XCTFail("expected FactoryError.unavailable, got \(error)")
      }
      ...
      XCTAssertTrue(
        hint.contains("InterfaceTransportFactories.rnode"),
  ...
  func testTheEntriesDiscoveryEmitsConstructOnRestart() throws {
    ...
    XCTAssertEqual(
      names, ["Discovered RNode", "Discovered KISS", "Discovered I2P"],
