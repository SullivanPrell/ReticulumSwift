...
final class SustainedBurstControlTests: XCTestCase {
    ...
    func testASustainedFloodKeepsTheBurstActivePastTheActivationHold() {
        let t = Transport()
        let t0 = Self.base()
        let iface = TestInterface(name: "sustained",
        ...
        XCTAssertTrue(t.shouldIngressLimit(on: iface, now: t0 + 31),
    ...
    func testActivatingABurstSeedsTheSustainedTimestamp() {
        let t = Transport()
        let t0 = Self.base()
        let iface = TestInterface(name: "seed",
                                  createdAt: Date(timeIntervalSince1970: t0 - Self.mature))
        t.register(interface: iface)
        ...
        XCTAssertEqual(t.ingressState(for: iface)?.burstSustained ?? 0, t0 + 0.5, accuracy: 0.001,
                       """
    ...
    func testAPathRequestBurstNeedsFourQuietCallsToClear() {
        let t = Transport()
        let t0 = Self.base()
        let iface = TestInterface(name: "pr-cooldown",
                                  createdAt: Date(timeIntervalSince1970: t0 - Self.mature))
        t.register(interface: iface)
        ...
        for (probe, remaining) in [(t0 + 40, 2), (t0 + 41, 1), (t0 + 42, 0)] {
            ...
            XCTAssertTrue(t.ingressState(for: iface)?.prBurstActive ?? false,
                          ...
                          """)
            XCTAssertEqual(t.ingressState(for: iface)?.prBurstCooldown, remaining,
    ...
    func testAnyNonQualifyingCallRefillsThePathRequestCooldown() {
        let t = Transport()
        let t0 = Self.base()
        let iface = TestInterface(name: "pr-refill",
                                  createdAt: Date(timeIntervalSince1970: t0 - Self.mature))
        t.register(interface: iface)
        ...
        XCTAssertEqual(t.ingressState(for: iface)?.prBurstCooldown, 3,
    ...
    func testTwoOutgoingSamplesAreEnoughToEgressLimit() {
        let t = Transport()
        let t0 = Self.base()
        let iface = TestInterface(name: "egress-floor",
                                  createdAt: Date(timeIntervalSince1970: t0 - Self.mature))
        ...
        XCTAssertTrue(t.shouldEgressLimitPR(on: iface, now: t0 + 0.2),
    ...
    func testTheRequestAboutToBeSentCountsTowardTheFrequency() {
        let t = Transport()
        let t0 = Self.base()
        let iface = TestInterface(name: "preemptive",
                                  createdAt: Date(timeIntervalSince1970: t0 - Self.mature))
        iface.egressControl = true
        t.register(interface: iface)
        ...
        XCTAssertEqual(t.outgoingPathRequestFrequency(for: iface, preemptive: false, now: t0 + 1.2),
                       5.0, accuracy: 0.0001,
                       "without the preemptive sample the stream sits exactly on the threshold")

        XCTAssertTrue(t.shouldEgressLimitPR(on: iface, now: t0 + 1.2),
                      """
    ...
    func testThePreemptiveSampleDoesNotSatisfyTheMinimumCountItself() {
        ...
        XCTAssertEqual(t.outgoingPathRequestFrequency(for: iface, preemptive: true, now: t0 + 0.01),
                       0, accuracy: 0.0001,
                       "one sample plus the preemptive one is still one recorded sample")
        XCTAssertFalse(t.shouldEgressLimitPR(on: iface, now: t0 + 0.01),
