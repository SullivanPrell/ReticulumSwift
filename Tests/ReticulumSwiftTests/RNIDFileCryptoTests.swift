...
final class RNIDFileCryptoTests: XCTestCase {
  ...
  func testChunkSizeIdentity() {
    ...
    XCTAssertEqual(
      RNIDFileCrypto.encryptedChunkSize(plaintextChunk: RNIDApp.encChunk),
      RNIDApp.decChunk)
  }
  ...
  func testRoundTripsSmallPayloads() throws {
    ...
    for size in [0, 1, 15, 16, 17, 1000] {
      ...
      try RNIDFileCrypto.decryptStream(
        identity: identity,
