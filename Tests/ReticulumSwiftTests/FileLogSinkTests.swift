...
final class FileLogSinkTests: XCTestCase {
  ...
  func testFormatLogLineWithoutTimestamps() {
    ...
    XCTAssertEqual(
      FileLogSink.formatLogLine("hello", level: .notice, timestamps: false, at: fixedDate),
      "[Notice]   hello")
  }
  ...
  func testFormatLogLineCompact() {
    ...
    XCTAssertEqual(
      FileLogSink.formatLogLine(
        "hello", level: .notice, timestamps: true,
        compact: true, at: fixedDate),
      "[\(fixedStamp)] hello")
    XCTAssertEqual(
      FileLogSink.formatLogLine(
        "hello", level: .notice, timestamps: false,
        compact: true, at: fixedDate),
      "hello")
  }
  ...
  func testFormatLogLinePrecise() {
    ...
    let line = FileLogSink.formatLogLine(
      "hello", level: .error, timestamps: false,
  ...
  func testFormatLogLineForEveryLevel() {
    ...
    for (level, name, gap) in expected {
      ...
      XCTAssertEqual(
        afterBracket.prefix(while: { $0 == " " }).count, gap,
        "wrong gap for \(name)")
    }
  }
  ...
  func testFallbackToConsoleOnWriteFailure() throws {
    ...
    XCTAssertTrue(
      captured[0].contains("Exception occurred while writing log message to log file"),
      captured[0])
  ...
  func testConcurrentWritesDoNotInterleave() {
    ...
    for line in lines {
      ...
      XCTAssertEqual(
        line.reversed().prefix(while: { $0 == "p" }).count, 40,
        "torn line: \(line)")
    }
  }
  ...
  func testInstallStdoutHandlerFormatsLikePython() {
    ...
    XCTAssertEqual(captured.count, 1)
    XCTAssertTrue(
      captured[0].hasSuffix("[Notice]   Started rnsd version \(Reticulum.version)"),
      captured[0])
  }
}
