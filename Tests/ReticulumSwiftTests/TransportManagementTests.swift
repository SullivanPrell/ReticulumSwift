...
final class TransportManagementTests: XCTestCase {
  ...
  func testGetPathTableMaxHopsFilter() throws {
    ...
    transport.restore(
      path: .init(
        destinationHash: h1, nextHopInterfaceName: "lo",
        hops: 1, lastHeard: Date(), identityHash: id.hash),
      forDestination: h1)
    transport.restore(
      path: .init(
        destinationHash: h2, nextHopInterfaceName: "lo",
  ...
  func testGetPathTableNilMaxHopsReturnsAll() throws {
    ...
    for i in UInt8(0)..<5 {
      ...
      transport.restore(
        path: .init(
          destinationHash: h, nextHopInterfaceName: "lo",
  ...
  func testAnEstablishedLinkIsRegisteredButAddsNoLinkTableEntry() throws {
    ...
    let a = LinkCountLoopback(name: "a")
    let b = LinkCountLoopback(name: "b")
    a.paired = b
    b.paired = a
