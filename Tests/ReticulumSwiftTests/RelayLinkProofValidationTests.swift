...
final class RelayLinkProofValidationTests: XCTestCase {
  ...
  func testAProofWithAnInvalidSignatureIsNotForwarded() throws {
    ...
    XCTAssertEqual(
      relay.towardInitiator.sent.count, 0,
      """
  ...
  func testAProofWithAnInvalidSignatureRaisesAProtocolViolation() throws {
    ...
    relay.transport.handleIncoming(
      packet: try proof(for: relay, signedBy: nil),
      from: relay.towardResponder)

    XCTAssertEqual(
      relay.transport.interfaceCounts(for: relay.towardResponder).protocolViolations,
      before + 1,
      """
  ...
  func testAValidlySignedProofIsForwardedToTheInitiator() throws {
    ...
    XCTAssertEqual(
      relay.towardInitiator.sent.count, 1,
    ...
    XCTAssertEqual(
      relay.transport.interfaceCounts(for: relay.towardResponder).protocolViolations, 0)
  }

  func testAValidlySignedProofMarksTheLinkTableEntryValidated() throws {
    let relay = makeRelay()
    XCTAssertEqual(
      relay.transport.linkRoutes[relay.linkID]?.validated, false,
      "an entry starts unvalidated—no proof has been checked yet")

    relay.transport.handleIncoming(
      packet: try proof(for: relay, signedBy: relay.responder),
      from: relay.towardResponder)

    XCTAssertEqual(
      relay.transport.linkRoutes[relay.linkID]?.validated, true,
  ...
  func testAProofArrivingFromTheInitiatorSideIsNotForwarded() throws {
    ...
    XCTAssertEqual(
      relay.towardResponder.sent.count, 0,
  ...
  func testAProofForADestinationWhoseIdentityIsUnknownIsNotForwarded() throws {
    ...
    XCTAssertEqual(
      ...
      """)
    XCTAssertEqual(
      t.interfaceCounts(for: towardResponder).protocolViolations, 0,
