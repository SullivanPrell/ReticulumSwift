...
final class AnnounceRateLimitTests: XCTestCase {
  ...
  func testFastPathTransmitsImmediatelyWithNoBitrateCap() {
    ...
    let ok = q.shouldTransmit(
      packet: pkt, now: 1_000, bitrate: 0, announceCap: AnnounceQueue.announceCap, emitted: 999)
    XCTAssertTrue(ok)
  ...
  func testFirstAnnounceTransmitsAndUpdatesAllowedAt() {
    ...
    let ok = q.shouldTransmit(
      packet: pkt, now: 1_000, bitrate: 9600, announceCap: AnnounceQueue.announceCap, emitted: 999)
    XCTAssertTrue(ok)
  ...
  func testSecondAnnounceDuringRateLimitIsQueued() {
    ...
    _ = q.shouldTransmit(
      packet: p1, now: 1_000, bitrate: 9600, announceCap: AnnounceQueue.announceCap, emitted: 999)
    // Second announce arrives before allowedAt—must be queued.
    let ok2 = q.shouldTransmit(
      packet: p2, now: 1_000.001, bitrate: 9600, announceCap: AnnounceQueue.announceCap,
      emitted: 999)
    XCTAssertFalse(ok2)
  ...
  func testDrainReleasesQueuedAnnouncesAfterWindow() {
    ...
    let drained = q.drain(
      now: q.allowedAt + 100, bitrate: 9600, announceCap: AnnounceQueue.announceCap)
  ...
  func testDuplicateDestinationKeepsFresherEntry() {
    ...
    _ = q.shouldTransmit(
      packet: old, now: 1_000, bitrate: 9600, announceCap: AnnounceQueue.announceCap, emitted: 900)
    // Same destination with a newer emitted ts—should replace old entry.
    _ = q.shouldTransmit(
      packet: new, now: 1_000.001, bitrate: 9600, announceCap: AnnounceQueue.announceCap,
      emitted: 950)
    XCTAssertEqual(q.count, 1)
    let drained = q.drain(
      now: q.allowedAt + 100, bitrate: 9600, announceCap: AnnounceQueue.announceCap)
