...
final class AnnounceCapTests: XCTestCase {
  ...
  func testFastInterfaceTransmitsImmediately() {
    ...
    let result = queue.shouldTransmit(
      packet: packet, now: now, bitrate: highBitrate, announceCap: AnnounceQueue.announceCap,
      emitted: 0)
  ...
  func testSecondAnnounceQueued_WhenBandwidthUsed() {
    ...
    let r1 = queue.shouldTransmit(
      packet: p1, now: now, bitrate: bitrate, announceCap: AnnounceQueue.announceCap, emitted: 0)
    let r2 = queue.shouldTransmit(
      packet: p2, now: now, bitrate: bitrate, announceCap: AnnounceQueue.announceCap, emitted: 0)
  ...
  func testAnnouncesTransmitAfterBandwidthWindow() {
    ...
    let drained = queue.drain(
      now: now + 1000, bitrate: bitrate, announceCap: AnnounceQueue.announceCap)
  ...
  func testAllowedAtAdvancedByCapWindowNoJitter() {
    ...
    _ = queue.shouldTransmit(
      packet: packet, now: now, bitrate: bitrate, announceCap: AnnounceQueue.announceCap, emitted: 0
    )
  ...
  func testQueueHoldsFarMoreThan16Announces() {
    ...
    for d in 0..<100 {
      _ = queue.shouldTransmit(
        packet: makeAnnounce(hops: 0, dest: UInt8(d)),
