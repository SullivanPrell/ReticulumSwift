...
final class HomeResolutionTests: XCTestCase {
  ...
  func testHomeDirectoryPrefersHOMEOverThePlatformHome() {
    ...
    XCTAssertEqual(
      InstanceConnection.homeDirectory(environment: ["HOME": relocated]).path,
      relocated)
  }
  ...
  func testExpandTildeHonoursHOME() {
    ...
    XCTAssertEqual(
      InstanceConnection.expandTilde("~", environment: env),
      "/tmp/rns-home-probe")
    XCTAssertEqual(
      InstanceConnection.expandTilde("~/.reticulum", environment: env),
      "/tmp/rns-home-probe/.reticulum")
    XCTAssertEqual(
      InstanceConnection.expandTilde("~/a/b", environment: env),
  ...
  func testExpandTildeMatchesExpanduserOnTheEdgeCases() {
    ...
    XCTAssertEqual(
      InstanceConnection.expandTilde("relative/path", environment: env),
      "relative/path")
  ...
  func testExpandTildeDisagreesWithThePlatformExpansionUnderARelocatedHOME() {
    let relocated = "/tmp/rns-home-probe"
    let ours = InstanceConnection.expandTilde(
      "~/.reticulum",
  ...
  func testRNCopyFileSystemHomeFollowsHOME() {
    ...
    XCTAssertEqual(
      fs.homeDirectoryPath, relocated,
      """
  ...
  func testRNCopyFileSystemStillFallsBackWhenHOMEIsUnset() {
    XCTAssertEqual(
      RNCopyDiskFileSystem(environment: [:]).homeDirectoryPath,
      NSHomeDirectory())
  }
    ...
    while let url = files?.nextObject() as? URL {
      ...
      for (index, line) in src.components(separatedBy: .newlines).enumerated() {
        ...
        for (pattern, why) in banned where code.contains(pattern) {
          ...
          offences.append(
            "  \(url.lastPathComponent):\(index + 1) — \(pattern) \(why)\n"
              + "      \(code)")
        }
      }
    }
