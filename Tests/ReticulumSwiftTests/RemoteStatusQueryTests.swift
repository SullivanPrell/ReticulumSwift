...
final class RemoteStatusQueryTests: XCTestCase {
  ...
  private func assertMalformed(
    _ payload: Data, _ message: String,
    file: StaticString = #filePath, line: UInt = #line
  ) {
    switch RemoteStatusQuery.decode(payload) {
    case .failure(let error):
      XCTAssertEqual(error, .malformedResponse, message, file: file, line: line)
  ...
  private func makeIdentifiedLink(allowClient: Bool = true) throws -> Fixture {
    ...
    let outbound = try Destination(
      identity: serverIdentity, direction: .out, kind: .single,
      appName: RNStatusApp.remoteManagementAppName,
      aspects: RNStatusApp.remoteManagementAspects)
  ...
  func testStatusRequestReturnsTheFullStatsDictAsANativeArray() throws {
    ...
    let receipt = try fixture.link.request(
      path: RNStatusApp.statusRequestPath,
    ...
    guard case .success(let (_, linkCount)) = RemoteStatusQuery.decode(try XCTUnwrap(payload))
    else {
      return XCTFail("expected success")
    }
  ...
  func testRequestBodyIsANativeArrayNotBytes() throws {
    ...
    if case .bytes = bytesReceived {
    } else {
      XCTFail(
        "the data: overload should wrap the payload as msgpack BIN, got \(String(describing: bytesReceived))"
      )
    }
  }
}
