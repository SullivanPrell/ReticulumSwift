...
final class MockRNXCommandExecutor: RNXCommandExecutor {
  ...
  var callCount: Int {
    lock.lock()
    defer { lock.unlock() }
    return unsafeCalls.count
  }

  var result: RNXExecution = RNXExecution(
    spawned: true, returnCode: 0,
  ...
  func execute(command: String, stdin: Data?, timeout: TimeInterval?) -> RNXExecution {
    lock.lock()
    unsafeCalls.append((command, stdin, timeout))
    lock.unlock()
    gate?.wait()
    return result
  }
...
final class RNXListenerTests: XCTestCase {
  ...
  private func makeListener(
    ...
    queue: DispatchQueue = DispatchQueue(label: "rnx.test")
  ) throws -> RNXListener {
    let listener = try RNXListener(
      identity: Identity(),
      transport: Transport(),
      executor: executor,
  ...
  func testResultForSuccessfulRun() throws {
    ...
    let execution = RNXExecution(
      spawned: true, returnCode: 0,
      stdout: Data("hi\n".utf8), stderr: Data())
    let result = listener.makeResult(
      for: request, execution: execution,
  ...
  func testConcludedTimestampRule() throws {
    ...
    let noTimeout = listener.makeResult(
      for: RNXRequest(command: "x"),
      execution: execution,
      startedAt: 1000, now: { 1000.5 })
    XCTAssertNil(noTimeout.concludedAt)

    // A command killed by the deadline also leaves it None.
    let killed = RNXExecution(
      spawned: true, returnCode: -15,
      stdout: Data(), stderr: Data(), timedOut: true)
    let timedOut = listener.makeResult(
      for: RNXRequest(command: "x", timeout: 5),
      execution: killed,
  ...
  func testStdoutLimit() throws {
    ...
    func result(_ limit: Int?) -> RNXResult {
      listener.makeResult(
        for: RNXRequest(command: "x", stdoutLimit: limit, stderrLimit: limit),
  ...
  func testNilOutputProducesAWellFormedResult() throws {
    ...
    let result = listener.makeResult(
      for: RNXRequest(command: "x", timeout: 1),
  ...
  func testSpawnFailureResult() throws {
    ...
    let listener = try makeListener()
    let result = listener.makeResult(
      for: RNXRequest(command: "", timeout: 15),
  ...
  func testParseAllowedHash() throws {
    ...
    XCTAssertThrowsError(try RNXListener.parseAllowedHash(String(repeating: "z", count: 32))) {
      XCTAssertEqual(
        $0 as? RNXListener.ListenerError,
        .invalidAllowedHashHex(String(repeating: "z", count: 32)))
    }
    // Python's bytes.fromhex accepts both cases.
    XCTAssertEqual(
      try RNXListener.parseAllowedHash(String(repeating: "AB", count: 16)),
  ...
  func testLoadAllowedIdentitiesFile() throws {
    ...
    let contents = "\r\n# a comment\r\n\(valid)\r\n0123456789\r\n\r\n"
    try contents.write(
      to: directory.appendingPathComponent(RNXApp.allowedIdentitiesFileName),
  ...
  func testLoadOrCreateIdentityCreatesMissingIdentitiesDirectory() throws {
    ...
    var logged: [String] = []
    let created = try RNXListener.loadOrCreateIdentity(at: url) { message, _ in
    ...
    logged.removeAll()
    let reloaded = try RNXListener.loadOrCreateIdentity(at: url) { message, _ in
      logged.append(message)
    }
