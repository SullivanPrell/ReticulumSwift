...
final class RNStatusRendererTests: XCTestCase {
  ...
  func testPathRequestsOnlyStillArrowsTheEmptyAnnounceStrings() {
    ...
    XCTAssertEqual(
      Self.renderer { $0.prStats = true }.render(stats: stats, linkCount: nil),
      "\n TCPInterface[Server on 0.0.0.0:4242]\n"
        + "    Status    : Up\n"
        + "    Mode      : Full\n"
        + "    Rate      : 10.00 Mbps\n"
  ...
  func testSharedInstanceSubtractsItsOwnAnnounceShare() {
    ...
    XCTAssertEqual(
      Self.renderer { $0.announceStats = true }.render(stats: stats, linkCount: nil),
  ...
  func testNoiseFloorVariants() {
    ...
    XCTAssertTrue(
      render([("noise_floor", .int(-110)), ("interference", .nil)])
        .contains("    Noise Fl. : -110 dBm, no interference\n"))
    // (c) interference reading present
    XCTAssertTrue(
      render([("noise_floor", .int(-110)), ("interference", .int(-95))])
  ...
  func testZeroInterferenceTakesTheLastSeenBranch() {
    ...
    let bare = Self.top([
      Self.baseInterface([("noise_floor", .int(-110)), ("interference", .int(0))])
    ])
    XCTAssertTrue(
      Self.renderer().render(stats: bare, linkCount: nil)
        .contains("    Noise Fl. : -110 dBm, no interference\n"))
  }
  ...
  func testAnnounceRateSuffixVariants() {
    func render(
      _ target: MsgPack.Value, _ penalty: MsgPack.Value, _ grace: MsgPack.Value,
      announceStats: Bool = true
    ) -> String {
      let stats = Self.top([
        Self.baseInterface([
  ...
  func testQueuedAndHeldAnnounces() {
    ...
    let zero = Self.top([
      Self.baseInterface([("announce_queue", .int(0)), ("held_announces", .int(0))])
    ])
    XCTAssertFalse(
      Self.renderer { $0.announceStats = true }
  ...
  func testLinkTableWithoutTransport() {
    ...
    XCTAssertTrue(
      Self.renderer { $0.linkStats = true }.render(stats: stats, linkCount: 12)
        .hasSuffix("\n 12 entries in link table\n\n"))
    // Without -l neither the count nor the line appears.
    XCTAssertTrue(
      Self.renderer().render(stats: stats, linkCount: 12)
  ...
  func testTotalsBlock() {
    ...
    XCTAssertTrue(
      Self.renderer { $0.trafficTotals = true }.render(stats: stats, linkCount: nil)
        .hasSuffix(
  ...
  func testHiddenInterfacesAreOmittedUnlessShowAll() {
    let stats = Self.top([
      Self.baseInterface([
        ("name", .string("Shared Instance[37428]")), ("clients", .int(2)),
        ("type", .string("LocalServerInterface")),
      ]),
      Self.baseInterface([
        ("name", .string("LocalInterface[52000]")),
        ("type", .string("LocalClientInterface")),
      ]),
      Self.baseInterface([
        ("name", .string("TCPInterface[Client on 1.2.3.4:4242]")),
