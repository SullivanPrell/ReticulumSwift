...
final class RNIDCommandLineTests: XCTestCase {
  ...
  func testSingleFlagsFromEachGroupAreAccepted() throws {
    XCTAssertEqual(
      try RNIDCommandLine.parse(["-s", "a", "-b", "-i", "abc"]).argumentValidation, .ok)
  }
  ...
  func testOutputFormatLadderOrder() {
    ...
    XCTAssertEqual(
      RNIDApp.outputFormat(base32: true, base64: true, base256: true, hex: true), .base32)
    XCTAssertEqual(
      RNIDApp.outputFormat(base32: false, base64: true, base256: true, hex: true), .base64)
    XCTAssertEqual(
      RNIDApp.outputFormat(base32: false, base64: false, base256: true, hex: true), .base256)
    XCTAssertEqual(
      RNIDApp.outputFormat(base32: false, base64: false, base256: false, hex: true), .hex)
    XCTAssertEqual(
      RNIDApp.outputFormat(base32: false, base64: false, base256: false, hex: false), .bin)
  }
  ...
  func testDestinationHashRejectsAWrongLengthIdentityHash() {
    ...
    XCTAssertNil(
      RNIDDestinationHash.hash(identityHash: Data(count: 8), appName: "rns", aspects: ["id"]))
  }
  ...
  func testUsageBlockMatchesArgparse() {
    let expected = """
      ...
      """
    XCTAssertTrue(
      RNIDCommandLine.helpText.hasPrefix(expected),
  ...
  func testOptionListLayout() {
    ...
    XCTAssertTrue(
      lines.contains("  --config path         path to alternative Reticulum config directory"))
    ...
    XCTAssertTrue(
      lines.contains(
        "                        hexadecimal Reticulum identity or destination hash, or"))
    XCTAssertTrue(lines.contains("                        path to Identity file"))
    // Exactly 20 characters, so it stays on one line.
    XCTAssertTrue(
      lines.contains("  -r path, --read path  input file path for operations with optional file"))
    XCTAssertTrue(
      lines.contains("  -t seconds            identity request timeout before giving up"))
  ...
  func testDescriptionSitsBetweenUsageAndOptions() {
    XCTAssertTrue(
      RNIDCommandLine.helpText.contains(
