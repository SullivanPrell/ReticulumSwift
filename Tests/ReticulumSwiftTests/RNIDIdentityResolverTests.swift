...
final class RNIDIdentityResolverTests: XCTestCase {
    ...
    -> (RNIDIdentityResolver, RNIDCapturingOutput, RNIDMemoryFileSystem)
  {
    ...
    let resolver = RNIDIdentityResolver(
      transport: transport, reticulum: nil,
  ...
  func testIdentityFileOfTheWrongLengthFails() {
    ...
    XCTAssertTrue(
      output.lines.first?.hasPrefix("Could not load Identity from specified file: ") ?? false)
  }
  ...
  func testRecallByDestinationHash() throws {
    ...
    let result = resolver.resolve(
      source: .identityArgument(RNIDEncoding.hexEncode(destinationHash)))
    ...
    XCTAssertEqual(
      output.lines,
      [
        "Recalled Identity \(RNSUtilities.prettyhexrep(source.hash))"
  ...
  func testRecallFallsBackToLocallyRegisteredDestinations() throws {
    ...
    let destination = try Destination(
      identity: source, direction: .in, kind: .single,
  ...
  func testUnknownHashWithoutRequestFails() {
    ...
    let result = resolver.resolve(
      source: .identityArgument(RNIDEncoding.hexEncode(unknown)),
  ...
  func testUnknownHashWithAllowNoneAndNoRequestReturnsNil() {
    ...
    let result = resolver.resolve(
      source: .identityArgument(String(repeating: "1", count: 32)),
  ...
  func testImportLadderAcceptsAllFourEncodings() throws {
    ...
    for (storedPath, argument, blob, isPrivate) in cases {
      ...
      let fileResult = fileResolver.resolve(
        source: isPrivate
          ? .importPrivate(argument)
      ...
      let hexResult = hexResolver.resolve(
        source: isPrivate ? .importPrivate(hex) : .importPublic(hex))
      ...
      let b32Result = b32Resolver.resolve(
        source: isPrivate
          ? .importPrivate(base32)
          : .importPublic(base32))
      ...
      let b64Result = b64Resolver.resolve(
        source: isPrivate
          ? .importPrivate(base64)
          : .importPublic(base64))
  ...
  func testImportPublicProducesAPublicOnlyIdentity() {
    ...
    let result = resolver.resolve(
      source: .importPublic(RNIDEncoding.hexEncode(source.getPublicKey())))
    guard case .success(let identity?) = result else { return XCTFail("expected an identity") }
  ...
  func testImportFailureMessagesDifferBetweenPublicAndPrivate() {
    ...
    guard case .failure(let privateCode) = privateResult else {
