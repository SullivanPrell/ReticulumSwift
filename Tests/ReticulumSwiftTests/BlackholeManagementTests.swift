...
final class BlackholeManagementTests: XCTestCase {
  ...
  func testBlackholeIdentityAddsToTable() {
    ...
    XCTAssertTrue(
      t.isBlackholed(identity.hash), "identity must be in blackhole table after blackholing")
  }
  ...
  func testRemoveBlackholedPathsKeepsUnaffectedPaths() throws {
    ...
    let good = Identity()
    let bad = Identity()
    let destGood = try Destination(
      identity: good, direction: .in, kind: .single,
      appName: "bh", aspects: ["good"])
    let destBad = try Destination(
      identity: bad, direction: .in, kind: .single,
      appName: "bh", aspects: ["bad"])

    t.restore(
      path: Transport.PathEntry(
        destinationHash: destGood.hash,
