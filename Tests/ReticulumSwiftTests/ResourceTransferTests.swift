...
final class ResourceTransferTests: XCTestCase {
  ...
  func makeLinkedPair() throws -> (
    aTransport: Transport, bTransport: Transport, aLink: Link, bLink: Link
  ) {
  ...
  func testResourceRoundTripSingleSegment() throws {
    ...
    bRx.bindAsReceiver()
    bRx.onPayloadReceived = { data, _ in
      got = data
  ...
  func testResourceMetadataTooLargeThrows() throws {
    ...
    XCTAssertThrowsError(try Resource(link: aLink, payload: Data("x".utf8), metadata: oversized)) {
      error in
