...
final class DestinationExpandNameTests: XCTestCase {
  ...
  func testExpandNameWithoutIdentity() {
    let name = Destination.expandName(
      identity: nil, appName: "myapp", aspects: ["service", "node"])
    XCTAssertEqual(
  ...
  func testLoadPublicKeyOnSingleReturnsFalse() throws {
    ...
    XCTAssertFalse(
      dest.loadPublicKey(fakeKey),
      "loadPublicKey on SINGLE must return false " + "(keys are held by the Identity instance)")
  }
  ...
  func testLoadPublicKeyOnPlainReturnsFalse() throws {
    let dest = try Destination(
      ...
      appName: "test", aspects: ["lpk"])
    XCTAssertFalse(
      dest.loadPublicKey(Data(repeating: 0, count: 32)),
  ...
  func testLoadPublicKeyOnGroupLoadsKey() throws {
    let dest = try Destination(
      identity: nil, direction: .in, kind: .group,
      appName: "test", aspects: ["lpk"])
