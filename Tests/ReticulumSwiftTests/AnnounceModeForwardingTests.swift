...
final class AnnounceModeForwardingTests: XCTestCase {
  ...
  private func injectAnnounce(
    destination: Destination,
    into transport: Transport,
    via receivingIface: ModeInterface
  ) throws {
  ...
  func testAPOutboundFromFullBlocked() {
    XCTAssertFalse(
      Transport.shouldForwardAnnounce(outboundMode: .accessPoint, nextHopMode: .full),
      "AP outbound must always be blocked")
  }
  func testAPOutboundFromRoamingBlocked() {
    XCTAssertFalse(
      Transport.shouldForwardAnnounce(outboundMode: .accessPoint, nextHopMode: .roaming))
  }
  func testAPOutboundFromAPBlocked() {
    XCTAssertFalse(
      Transport.shouldForwardAnnounce(outboundMode: .accessPoint, nextHopMode: .accessPoint))
  }
  func testAPOutboundFromBoundaryBlocked() {
    XCTAssertFalse(
      Transport.shouldForwardAnnounce(outboundMode: .accessPoint, nextHopMode: .boundary))
  }
  ...
  func testFullOutboundFromAnyForwards() {
    for nhm: InterfaceMode in [.full, .gateway, .roaming, .accessPoint, .boundary, .pointToPoint] {
      XCTAssertTrue(
        Transport.shouldForwardAnnounce(outboundMode: .full, nextHopMode: nhm),
  ...
  func testGatewayOutboundFromAnyForwards() {
    for nhm: InterfaceMode in [.full, .gateway, .roaming, .accessPoint, .boundary, .pointToPoint] {
      XCTAssertTrue(
        Transport.shouldForwardAnnounce(outboundMode: .gateway, nextHopMode: nhm),
  ...
  func testP2POutboundFromAnyForwards() {
    for nhm: InterfaceMode in [.full, .gateway, .roaming, .accessPoint, .boundary, .pointToPoint] {
      XCTAssertTrue(
        Transport.shouldForwardAnnounce(outboundMode: .pointToPoint, nextHopMode: nhm),
  ...
  func testRoamingOutboundFromAPForwards() {
    XCTAssertTrue(
      Transport.shouldForwardAnnounce(outboundMode: .roaming, nextHopMode: .accessPoint),
      "ROAMING outbound must forward when next-hop is AP (not ROAMING/BOUNDARY)")
  }
  func testRoamingOutboundFromRoamingBlocked() {
    XCTAssertFalse(
      Transport.shouldForwardAnnounce(outboundMode: .roaming, nextHopMode: .roaming),
      "ROAMING outbound must block when next-hop is also ROAMING")
  }
  func testRoamingOutboundFromBoundaryBlocked() {
    XCTAssertFalse(
      Transport.shouldForwardAnnounce(outboundMode: .roaming, nextHopMode: .boundary),
  ...
  func testBoundaryOutboundFromAPForwards() {
    XCTAssertTrue(
      Transport.shouldForwardAnnounce(outboundMode: .boundary, nextHopMode: .accessPoint),
      "BOUNDARY outbound must forward when next-hop is AP")
  }
  func testBoundaryOutboundFromBoundaryForwards() {
    XCTAssertTrue(
      Transport.shouldForwardAnnounce(outboundMode: .boundary, nextHopMode: .boundary),
      "BOUNDARY outbound must forward when next-hop is BOUNDARY")
  }
  func testBoundaryOutboundFromRoamingBlocked() {
    XCTAssertFalse(
      Transport.shouldForwardAnnounce(outboundMode: .boundary, nextHopMode: .roaming),
  ...
  func testFullReceivingDoesNotForwardToAP() throws {
    ...
    XCTAssertEqual(
      gatewayOut.sent.filter { $0.packetType == .announce }.count, 1,
      "FULL receive must forward to GATEWAY outbound")
    XCTAssertEqual(
      roamingOut.sent.filter { $0.packetType == .announce }.count, 1,
      "FULL receive must forward to ROAMING outbound (next-hop is FULL, not ROAMING/BOUNDARY)")
    XCTAssertEqual(
      apOut.sent.filter { $0.packetType == .announce }.count, 0,
  ...
  func testRoamingReceivingBlocksRoamingAndBoundaryOutbound() throws {
    ...
    XCTAssertEqual(
      fullOut.sent.filter { $0.packetType == .announce }.count, 1,
      "ROAMING receive must forward to FULL outbound")
    XCTAssertEqual(
      roamOut.sent.filter { $0.packetType == .announce }.count, 0,
      "ROAMING receive must NOT forward to another ROAMING outbound")
    XCTAssertEqual(
      boundaryOut.sent.filter { $0.packetType == .announce }.count, 0,
