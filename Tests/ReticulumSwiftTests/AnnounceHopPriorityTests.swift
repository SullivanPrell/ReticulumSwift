...
final class AnnounceHopPriorityTests: XCTestCase {
  ...
  func testQueueDrainsLowHopFirst() {
    ...
    _ = queue.shouldTransmit(
      packet: first, now: now, bitrate: bitrate, announceCap: AnnounceQueue.announceCap, emitted: 0)
    ...
    _ = queue.shouldTransmit(
      packet: highHop, now: now, bitrate: bitrate, announceCap: AnnounceQueue.announceCap,
      emitted: 0)
    _ = queue.shouldTransmit(
      packet: lowHop, now: now, bitrate: bitrate, announceCap: AnnounceQueue.announceCap, emitted: 0
    )
    ...
    guard drained.count >= 2 else {
      XCTFail("expected at least 2 drained, got \(drained.count)")
      return
    }
