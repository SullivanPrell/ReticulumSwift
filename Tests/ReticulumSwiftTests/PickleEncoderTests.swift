...
final class PickleEncoderTests: XCTestCase {
  ...
  func testFloatZero() {
    ...
    XCTAssertEqual(
      data, Data([0x80, 0x04, 0x47, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2e]))
  }
  ...
  func testFloatOne() {
    ...
    XCTAssertEqual(
      data, Data([0x80, 0x04, 0x47, 0x3f, 0xf0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2e]))
  }
  ...
  func testDecoderBytes16() {
    ...
    let payload = PickleEncoder.encode(
      .stringDict([
        ("get", .string("next_hop")),
  ...
  func testMsgPackRPCPathTableCall() throws {
    ...
    let kv = Dictionary(
      uniqueKeysWithValues: pairs.compactMap { k, v -> (String, MsgPack.Value)? in
  ...
  func testMsgPackRPCNextHopCall() throws {
    ...
    guard case .bytes(let gotHash) = kv["destination_hash"] else {
      XCTFail()
      return
    }
