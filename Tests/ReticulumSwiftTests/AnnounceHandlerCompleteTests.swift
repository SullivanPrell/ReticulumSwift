...
final class AnnounceHandlerCompleteTests: XCTestCase {
  ...
  final class RecordingHandler: AnnounceHandler {
    ...
    var received:
      [(destinationHash: Data, identity: Identity, appData: Data?, isPathResponse: Bool)] = []
  ...
  func testHandlerWithAspectFilterOnlyReceivesMatching() throws {
    ...
    let bId = Identity()
    let lxmfDest = try Destination(
      identity: bId, direction: .in, kind: .single,
      appName: "lxmf", aspects: ["delivery"])
    let otherDest = try Destination(
      identity: bId, direction: .in, kind: .single,
  ...
  struct Transports {
    let a: Transport
    let b: Transport
    let aIface: LoopbackInterface
    let bIface: LoopbackInterface
  }
