...
final class ResourceProgressCallbackTests: XCTestCase {
  ...
  private func establishLink(
    ...
    aI.paired = bI
    bI.paired = aI
    aI.asynchronous = asynchronous
    bI.asynchronous = asynchronous
  ...
  func testRequestReceiptReportsResponseSizeMidTransfer() throws {
    ...
    _ = try aLink.request(
      ...
      progressCallback: { progress, receipt in
        ...
        lock.lock()
        if midTransferSize == nil {
          midTransferSize = size
          midTransferProgress = progress
        }
        lock.unlock()
        sawSize.fulfill()
      },
    ...
    lock.lock()
    let size = midTransferSize
    let progress = midTransferProgress
    lock.unlock()
    let observedSize = try XCTUnwrap(size)
    XCTAssertGreaterThanOrEqual(
      observedSize, payload.count,
