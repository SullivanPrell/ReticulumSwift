...
final class LinkTableCountTests: XCTestCase {
  ...
  private func wire(_ a: HopInterface, _ b: HopInterface) {
    a.paired = b
    b.paired = a
  }
  ...
  func testADirectLinkAddsNothingToEitherSideLinkTable() throws {
    ...
    let a = HopInterface(name: "a")
    let b = HopInterface(name: "b")
    wire(a, b)
    aT.register(interface: a)
    bT.register(interface: b)

    let aE = expectation(description: "initiator")
    let bE = expectation(description: "responder")
  ...
  func testTheCountTracksLinkTableEntriesAsTheyAreSwept() throws {
    ...
    t.restore(
      linkRoute: Transport.LinkRoute(
        linkID: Data(repeating: 0x01, count: 16),
