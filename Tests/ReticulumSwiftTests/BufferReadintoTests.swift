...
final class BufferReadintoTests: XCTestCase {
  ...
  private final class LoopbackOutlet: ChannelOutlet {
    ...
    func getPacketState(_ h: ChannelPacketHandle) -> MessageState {
      h.state == .delivered ? .delivered : .sent
    }
    func timedOut() {}
    func setPacketTimeoutCallback(
      _ h: ChannelPacketHandle, timeout: TimeInterval?, callback: ((ChannelPacketHandle) -> Void)?
    ) {
      guard let t = timeout, let cb = callback else {
        h.setTimeoutWork(nil)
        return
      }
    ...
    func setPacketDeliveredCallback(
      _ h: ChannelPacketHandle, callback: ((ChannelPacketHandle) -> Void)?
    ) {
