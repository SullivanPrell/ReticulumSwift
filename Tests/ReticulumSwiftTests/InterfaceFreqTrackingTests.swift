...
final class InterfaceFreqTrackingTests: XCTestCase {
  ...
  func testArFreqDecayConstant() {
    XCTAssertEqual(
      InterfaceFreqTracker.arFreqDecay, 10.0, accuracy: 0.001,
      "AR_FREQ_DECAY = 1/AR_MINFREQ_HZ = 1/0.1 = 10")
  }

  func testPrFreqDecayConstant() {
    XCTAssertEqual(
      InterfaceFreqTracker.prFreqDecay, 10.0, accuracy: 0.001,
  ...
  func testMaxSamplesConstant() {
    XCTAssertEqual(
      InterfaceFreqTracker.maxSamples, 48,
  ...
  func testZeroSpanReturnsZero() {
    ...
    t.recordIncomingAnnounce(at: 5)
    XCTAssertEqual(
      t.incomingAnnounceFrequency(now: 5), 0.0,
