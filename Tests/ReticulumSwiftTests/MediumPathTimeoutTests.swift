...
final class MediumPathTimeoutTests: XCTestCase {
  ...
  func testABitrateBelowTheFloorIsClampedToMinimumBitrate() {
    ...
    t.prioritizeInterfaces()
    XCTAssertEqual(
      t.mediumPathTimeout(), expected(bitrate: Transport.minimumBitrate), accuracy: 1e-9)
    XCTAssertEqual(
      t.lowestInterfaceBitrate, 1, "the clamp belongs to the timeout, not to the recorded bitrate")
  }
  ...
  func testTheLastKnownBitrateSurvivesEveryInterfaceGoingOffline() {
    ...
    t.prioritizeInterfaces()
    XCTAssertEqual(
      t.lowestInterfaceBitrate, 1200, "a failed recompute must not clear the last known value")
  }
