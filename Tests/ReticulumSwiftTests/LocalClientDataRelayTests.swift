...
final class LocalClientDataRelayTests: XCTestCase {
  ...
  private func dataPacket(
    to dest: Data, headerType: Packet.HeaderType = .type1,
  ...
  func testMeshDataDeliveredToLocalClientWhenTransportDisabled() throws {
    ...
    mesh.inboundHandler?(
      dataPacket(to: dest.hash, headerType: .type2, transportID: t.transportInstanceID), mesh)

    let delivered = serving.sent.filter {
      $0.destinationHash == dest.hash && $0.packetType == .data
    }
    XCTAssertEqual(
      delivered.count, 1,
  ...
  func testNonTransportNodeDoesNotRelayMeshData() throws {
    ...
    t.injectPath(
      dest.hash, nextHop: Data(repeating: 0xCC, count: 16),
      receivedOn: meshA, hops: 2, announcePacketHash: nil)
    meshA.sent.removeAll()
    meshB.sent.removeAll()

    // DATA for F arrives from meshB. This node isn't transport, and it's not from or for a
    // local client → must NOT relay.
    meshB.inboundHandler?(
      dataPacket(to: dest.hash, headerType: .type2, transportID: t.transportInstanceID), meshB)
