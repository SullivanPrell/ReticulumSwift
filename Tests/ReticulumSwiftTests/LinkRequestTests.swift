...
final class LinkRequestTests: XCTestCase {
  ...
  func testLargeResponseViaResourceNativeHandler() throws {
    ...
    bDestination.registerNativeRequestHandler(path: "/page/large.mu", allow: .all) {
      _, _, _, _, _ in
  ...
  func testRequestHandlerAllowListBlocksUnidentifiedPeer() throws {
    ...
    bDestination.registerRequestHandler(
      path: "restricted", allow: .list, allowedList: [someIdentity]
    ) { _, _, _, _, _ in
      handlerCalled.fulfill()
      return Data("ok".utf8)
    }
