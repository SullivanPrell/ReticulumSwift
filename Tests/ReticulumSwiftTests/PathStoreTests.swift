...
final class PathStoreTests: XCTestCase {
  ...
  func testRoundTripThroughFile() throws {
    ...
    let installed = try installPersistablePath(
      on: live, through: eth0, hops: 3,
      aspect: "roundtrip")
  ...
  func testIdentityIsResolvedFromKnownDestinationsNotTheEntry() throws {
    ...
    XCTAssertNotNil(
      withoutIdentity.paths[installed.destinationHash],
  ...
  func testRandomBlobsSurviveRoundTrip() throws {
    ...
    XCTAssertEqual(
      revived.paths[installed.destinationHash]?.randomBlobs, blobs,
  ...
  func testRandomBlobsCappedAtPersistLimit() throws {
    ...
    XCTAssertEqual(
      revived.paths[installed.destinationHash]?.randomBlobs,
