...
final class KnownDestinationsParityTests: XCTestCase {
  ...
  func testFileIsMsgpackDictOfFiveElementLists() throws {
    ...
    XCTAssertEqual(
      fields[2], .bytes(identity.publicKeyBytes),
      """
  ...
  func testLastUseSentinelsAreWritten() throws {
    ...
    guard let used = entries[usedHash], used.count == 5,
      let retained = entries[retainedHash], retained.count == 5
    else {
  ...
  func testFourElementEntryIsBackfilled() throws {
    ...
    let legacy = MsgPack.Value.map([
      (
        .bytes(destHash),
        .array([
          .double(1_700_000_000), .bytes(Data(repeating: 0x01, count: 32)),
    ...
    XCTAssertEqual(
      transport.recall(identity: destHash)?.publicKeyBytes,
  ...
  func testEntryWithWrongKeyLengthIsSkipped() throws {
    ...
    XCTAssertNotNil(
      transport.recall(identity: goodHash),
  ...
  func testRoundTrip() throws {
    ...
    XCTAssertEqual(
      revived.knownDestinationAnnouncedAt[usedHash]?.timeIntervalSince1970 ?? 0,
      announcedAt.timeIntervalSince1970, accuracy: 0.001)
    XCTAssertEqual(
      revived.knownDestinationLastUsed[usedHash]?.timeIntervalSince1970 ?? 0,
      usedAt.timeIntervalSince1970, accuracy: 0.001)
    XCTAssertNil(
      revived.knownDestinationLastUsed[retainedHash],
      "the -1 sentinel is retention, not a use time")
    XCTAssertTrue(
      revived.retainedDestinations.contains(retainedHash),
