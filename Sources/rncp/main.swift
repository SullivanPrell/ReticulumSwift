...
func converted<T>(
  _ name: String, typeName: String, default defaultValue: T,
  convert: (String) -> T?
) -> T {
...
let timeout = converted("-w", typeName: "float", default: Transport.pathRequestTimeout) {
  Double($0)
}
...
func startReticulum() -> InstanceConnection {
  ...
  do {
    return try InstanceConnection.attach(
      configDirectory: configArgument.map {
        URL(fileURLWithPath: RNCopyApp.expandUser($0, home: fileSystem.homeDirectoryPath))
      },
...
func prepareIdentity(connection: InstanceConnection) -> Identity {
  ...
  if let identityArgument {
    url = URL(
      fileURLWithPath: RNCopyApp.expandUser(identityArgument, home: fileSystem.homeDirectoryPath))
  } else {
...
func runListen() -> Never {
  ...
  if noAuth {
    ...
    let load = RNCopyApp.loadAllowedIdentities(
      commandLineEntries: allowedArguments,
      fileSystem: fileSystem)
    if let failure = load.failure {
      Reticulum.log(
        "Error while parsing allowed_identities file. The contained exception was: \(failure)",
        level: .error)
    }
  ...
  if allowedIdentityHashes.isEmpty && !noAuth {
    Reticulum.log(
      "No allowed identities configured, rncp will not accept any files!",
      level: .warning)
  }
...
func runSend(file: String, destination: String) -> Never {
  ...
  let sender = RNCopySender(
    transport: connection.reticulum.transport,
    fileSystem: fileSystem,
    configuration: RNCopySender.Configuration(
      identity: identity,
  ...
  sender.onStage = { stage in
    ...
    case .requestingPath:
      if silent {
        emit("Path to \(prettyDestination) requested")
      } else {
        emit("Path to \(prettyDestination) requested  ", terminator: endSpace)
      }
    case .establishingLink:
      if silent {
        emit("Establishing link with \(prettyDestination)")
      }
      // NOTE: send appends ONE literal space here; fetch appends TWO.
      else {
        emit("\(erase)Establishing link with \(prettyDestination) ", terminator: endSpace)
      }
    case .advertising:
      if silent {
        emit("Advertising file resource...")
      } else {
        emit("\(erase)Advertising file resource  ", terminator: endSpace)
      }
    case .transferring:
      if silent {
        emit("Transferring file...")
      } else {
        emit("\(erase)Transferring file  ", terminator: endSpace)
      }
  ...
  sender.onProgress = { progress in
    ...
    let stat = RNCopyApp.transferStat(
...
func runFetch(file: String, destination: String) -> Never {
  ...
  let fetcher = RNCopyFetcher(
    transport: connection.reticulum.transport,
    fileSystem: fileSystem,
    configuration: RNCopyFetcher.Configuration(
      identity: identity,
      destinationHash: destinationHash,
      remotePath: file,
      timeout: timeout,
  ...
  fetcher.onStage = { stage in
    ...
    case .requestingFile:
      if silent {
        emit("Requesting file from remote...")
      } else {
        emit("\(erase)Requesting file from remote  ", terminator: endSpace)
      }
  ...
  case .requestFailed(let status):
    ...
    case .fetchNotAllowed:
      concludeRequestFailure(
        "Fetch request failed, fetching the file \(file) was not allowed by the remote")
    case .notFound:
    ...
    case .unknown, .found:
      concludeRequestFailure(
        "Fetch request failed due to an unknown error (probably not authorised)")
    }
