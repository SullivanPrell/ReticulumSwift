...
public final class AutoInterface: Interface {
  ...
  private func discoverInterfaces() {
    ...
    while let current = ifa {
      ...
      current.pointee.ifa_addr.withMemoryRebound(to: sockaddr.self, capacity: 1) { sa in
        _ = getnameinfo(
          sa, socklen_t(MemoryLayout<sockaddr_in6>.size),
  ...
  private func setupDataSocket() throws {
    ...
    setsockopt(
      s, SOL_SOCKET, SO_REUSEADDR, &reuseVal, socklen_t(MemoryLayout.size(ofValue: reuseVal)))
    setsockopt(
      s, SOL_SOCKET, SO_REUSEPORT, &reuseVal, socklen_t(MemoryLayout.size(ofValue: reuseVal)))
    ...
    guard bound == 0 else {
      Darwin.close(s)
      throw AutoInterfaceError.socketError("bind data")
    }
    dataSocket = s
  }
  ...
  private func setupDiscoverySocket() throws {
    ...
    guard bound == 0 else {
      Darwin.close(s)
      throw AutoInterfaceError.socketError("bind discovery")
    }
  ...
  private func sendBeacon(ifname: String, linkLocal: String) {
    ...
    setsockopt(
      s, IPPROTO_IPV6, IPV6_MULTICAST_IF, &mcastIfIdx, socklen_t(MemoryLayout<UInt32>.size))
  ...
  private func addPeer(addr: String, ifname: String) {
    ...
    if let existing = peers[addr] {
      peers[addr] = (
        ifname: existing.ifname, lastHeard: Date(), lastOutbound: existing.lastOutbound
      )
    } else {
  ...
  private func startPeerJobLoop() {
    let timer = DispatchSource.makeTimerSource(queue: queue)
    timer.schedule(
      deadline: .now() + AutoInterface.peerJobInterval,
  ...
  private func peerJob() {
    ...
    peersLock.lock()
    peers = peers.filter {
      now.timeIntervalSince($0.value.lastHeard) < AutoInterface.peeringTimeout
    }
    peersLock.unlock()
  }
  ...
  private func sendDataToPeer(addr: String, ifname: String, data: Data) {
    ...
    data.withUnsafeBytes { ptr in
      _ = sendto(
        dataSocket, ptr.baseAddress, ptr.count, 0,
