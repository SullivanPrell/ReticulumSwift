...
public final class I2PInterfacePeer: Interface, SpawnedInterface {
  ...
  /// Idle seconds before keepalives flow.
  ///
  /// Python: `I2P_PROBE_AFTER`.
  ...
  /// Watchdog cadence.
  ///
  /// Python's read_watchdog ticks every 1 s.
  ...
  /// Begin dialing the remote destination through SAM.
  ///
  /// Non-blocking; the
  ...
  public func processOutgoing(_ data: Data) {
    lock.lock()
    guard isOnline, let stream = streamSocket else {
      lock.unlock()
      return
    }
    lastWrite = Date()
  ...
  private func dialOnce() {
    ...
    do {
      ...
      Reticulum.log(
        "Check that I2P is running and SAM is enabled. Retrying tunnel setup later.", level: .error)
      cleanupSockets()
      lock.lock()
      let retry = !detached
      lock.unlock()
  ...
  private func performDial() throws {
    Reticulum.log(
      "Bringing up I2P tunnel to \(displayName), this may take a while...", level: .info)
  ...
  private func handleStreamBytes(_ data: Data, from socket: SAMSocket) {
    lock.lock()
    guard streamSocket === socket else {
      lock.unlock()
      return
    }
    lastRead = Date()
  ...
  private func disconnect(ifCurrent socket: SAMSocket?, reason: String) {
    lock.lock()
    if let socket, streamSocket !== socket {
      lock.unlock()
      return
    }
    guard isOnline else {
