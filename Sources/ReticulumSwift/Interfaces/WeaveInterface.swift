...
public final class WDCLTransport {
  ...
  public func open(port: String) throws {
    try transport.open(
      port: port, baudRate: WDCLTransport.speed,
...
public final class WeaveDevice {
  ...
  public func handshake() {
    guard let conn = connection, let remoteID = switchID,
      let signature = try? conn.sign(remoteID)
    else { return }
  ...
  public func incomingFrame(_ data: Data) {
    ...
    case WDCL.tDiscover:
      ...
      guard let pubKey = try? Curve25519.Signing.PublicKey(rawRepresentation: remotePubBytes),
        pubKey.isValidSignature(remoteSig, for: signedID)
      else { return }
    ...
    case WDCL.tLog:
      ...
      let ts =
        UInt32(payload[1]) << 24 | UInt32(payload[2]) << 16
  ...
  public func handleLog(_ frame: WeaveLogFrame) {
    ...
    case WeaveEvt.etStatMemory where frame.data.count >= 8:
      memFree = Int(
        UInt32(frame.data[0]) << 24 | UInt32(frame.data[1]) << 16
          | UInt32(frame.data[2]) << 8 | UInt32(frame.data[3]))
      memTotal = Int(
        UInt32(frame.data[4]) << 24 | UInt32(frame.data[5]) << 16
...
public final class WeaveInterface: Interface {
  ...
  public func peerJobs() {
    ...
    where now.timeIntervalSince(endpoint.lastSeen) > WeaveInterface.peeringTimeout {
      expired.append(addr)
    }
    for addr in expired {
