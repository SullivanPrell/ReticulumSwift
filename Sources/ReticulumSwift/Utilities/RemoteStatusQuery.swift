...
public final class RemoteStatusQuery {
  ...
  public func request(
    ...
    if destination == nil {
      guard let remoteIdentity = transport.recall(identity: destinationHash) else {
        completion(.failure(.noRemoteIdentity))
        return
      }
    ...
    guard let destination,
      let fresh = try? Link.initiate(destination: destination, transport: transport)
    else {
      completion(.failure(.linkClosedUnexpectedly))
      return
    }
    lock.lock()
    link = fresh
  ...
  public func requestBlocking(
    includeLinkStats: Bool,
    progress: ((String) -> Void)? = nil
  ) throws -> (MsgPack.Value, Int?) {
