...
extension RNCopyApp {
  ...
  public static func saveReceivedResource(
    payload: Data,
    metadata: Data?,
    savePath: String?,
    allowOverwrite: Bool,
    ...
    -> Swift.Result<String, RNCopyError>
  {
    ...
    let resolution = resolveSaveTarget(
      filename: filename,
      savePath: savePath,
...
public final class RNCopyListener {
  ...
  private func acceptResource(advertisement: ResourceAdvertisement, from link: Link) -> Bool {
    ...
    if accepted {
      stateLock.lock()
      lastAcceptedAdvertisement = advertisement
      stateLock.unlock()
    }
    return accepted
  }
  ...
  private func handleResourceStarted(_ transfer: ResourceTransfer) {
    ...
    Reticulum.log(
      "Starting resource transfer " + RNSUtilities.prettyhexrep(resourceHash) + idString,
      level: .info)
  ...
  private func handleResourceConcluded(
    payload: Data,
    ...
    Reticulum.log(
      "Incoming resource <\(advertisement.resourceHash.hexString)/\(linkID.hexString)> completed",
      level: .info)
    ...
    case .failure(let error):
      ...
      case .writeFailed, .overwriteFailed:
        Reticulum.log(
          "An error occurred while saving received resource: \(error.exceptionText)",
          level: .error)
      }
    }
  ...
  public func saveReceivedResource(payload: Data, metadata: Data?)
    -> Swift.Result<String, RNCopyError>
  {
    RNCopyApp.saveReceivedResource(
      ...
      onOverwriteFailure: { path in
        ...
        Reticulum.log(
          "Could not overwrite existing file \(path), renaming instead",
          level: .error)
      }
    )
  }
  ...
  public func serveFetchRequest(requested: String, link: Link) -> MsgPack.Value? {
    ...
    if !configuration.allowAll {
      guard let remoteHash = link.getRemoteIdentity()?.hash,
        configuration.allowedIdentityHashes.contains(remoteHash)
      else { return nil }
    }

    switch RNCopyApp.resolveFetchPath(
      requested: requested,
    ...
    case .notAllowed(let path):
      Reticulum.log(
        "Disallowing fetch request for \(path) outside of fetch jail \(configuration.fetchJail ?? "")",
        level: .warning)
    ...
    case .serve(let path):
      ...
      do {
        ...
        Reticulum.log(
          "Could not send file to client. The contained exception was: \(error)",
          level: .error)
        return .bool(false)
      }
    }
