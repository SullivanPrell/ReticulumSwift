...
public struct RNCopyProgress: Equatable {
  ...
  public init(
    fraction: Double, transferredBytes: Int, totalBytes: Int,
...
final class RNCopyLinkOpener {
  ...
  init(
    transport: Transport, destinationHash: Data,
  ...
  func open(onStage: ((RNCopyStage) -> Void)?, onTick: (() -> Void)?) -> Outcome {
    ...
    do {
      let listenerDestination = try Destination(
...
public final class RNCopySender {
  ...
  public func run() -> Outcome {
    ...
    let payload: Data
    do { payload = try fileSystem.readFile(atPath: filePath) } catch {
    ...
    do { try link.identify(as: configuration.identity) } catch { return .linkTimedOut }
...
public final class RNCopyFetcher {
  ...
  public func run() -> Outcome {
    ...
    do { try link.identify(as: configuration.identity) } catch { return .linkFailed }
