...
public struct RNStatusRenderer {
  ...
  public func render(stats: RNStatusStats, linkCount: Int?, activeLinkCount: Int? = nil) -> String {
    ...
    for ifstat in ordered where !RNStatusStats.shouldHide(ifstat, showAll: options.showAll) {
      guard
        RNStatusStats.passesFilters(
          ifstat,
          nameFilter: options.nameFilter,
          burstFilter: options.burstFilter)
      else { continue }
    ...
    if options.trafficTotals {
        ...
        stats.has("prxs"), stats.has("ptxs"), stats.has("arxs"), stats.has("atxs")
      {
        ...
        if stats.rxs > 0 {
          rxstat +=
            ", \(Self.pythonInt(drxpct))% data (\(RNSUtilities.prettyspeed(stats.rxs * (drxpct / 100.0))))"
        }
        if stats.txs > 0 {
          txstat +=
            ", \(Self.pythonInt(dtxpct))% data (\(RNSUtilities.prettyspeed(stats.txs * (dtxpct / 100.0))))"
        }
      }
      ...
      {
        out += Self.aggregateBlock(
          label: "Path Rqs.", stats: stats,
          rxb: "prxb", txb: "ptxb",
          rxs: "prxs", txs: "ptxs",
          rxf: "prxf", txf: "ptxf")
      }
      if options.announceStats, stats.has("arxb"), stats.has("atxb"), stats.has("arxs"),
        stats.has("atxs")
      {
        out += Self.aggregateBlock(
          label: "Announces", stats: stats,
          rxb: "arxb", txb: "atxb",
          rxs: "arxs", txs: "atxs",
          rxf: "arxf", txf: "atxf")
      }
    ...
    if options.queueStats {
      func line(_ pressure: String, _ label: String, _ depth: String, _ dropped: String) -> String {
        ...
        return
          "\(Self.pythonRound(stats.double(pressure) * 100.0, 1))% \(label), \(stats.int(depth)) pkts\(suffix)"
      }
      out += "\n Qu. Pressure : \(line("tqpressure", "total", "rxqt", "rxqtd"))\n"
      for row in [
        line("dqpressure", "data", "rxqd", "rxqdd"),
  ...
  public func renderInterface(_ ifstat: RNStatusInterfaceStats) -> String {
    ...
    if let count = clients {
      if name.hasPrefix("Shared Instance[") {
        ...
        if ifstat.has("i2p_connectable"), ifstat.bool("i2p_connectable") == true {
          clientsString =
            "Peers     : \(count)\(count == 1 ? " connected I2P endpoint" : " connected I2P endpoints")"
        } else {
          clientsString = ""
        }
      } else {
    ...
    if ifstat.has("airtime_short"), ifstat.has("airtime_long") {
      out +=
        "    Airtime   : \(Self.pythonStr(ifstat.raw("airtime_short") ?? .nil))% (15s), "
        + "\(Self.pythonStr(ifstat.raw("airtime_long") ?? .nil))% (1h)\n"
    }
    if ifstat.has("channel_load_short"), ifstat.has("channel_load_long") {
      out +=
        "    Ch. Load  : \(Self.pythonStr(ifstat.raw("channel_load_short") ?? .nil))% (15s), "
    ...
    let target =
      options.announceStats && ifstat.has("announce_rate_target")
      ? ifstat.raw("announce_rate_target") : nil
    let penalty =
      options.announceStats && ifstat.has("announce_rate_penalty")
      ? ifstat.raw("announce_rate_penalty") : nil
    let grace =
      options.announceStats && ifstat.has("announce_rate_grace")
      ? ifstat.raw("announce_rate_grace") : nil
    ...
    if hasTarget && hasPenalty && hasGrace {
      artStr =
        "(t:\(Self.prettytime(target))/p:\(Self.prettytime(penalty))"
      ...
      let incoming = ifstat.double("incoming_announce_frequency")
    {
      ...
      if ifstat.has("prxs"), ifstat.has("rxs"), ifstat.has("ptxs"), ifstat.has("txs") {
        let apctstr =
          "(↓\(Self.flowPercent(ifstat, "arxs", "rxs"))% / ↑\(Self.flowPercent(ifstat, "atxs", "txs"))% of flow)"
      ...
      let incoming = ifstat.double("incoming_pr_frequency")
    {
      ...
      if ifstat.has("prxs"), ifstat.has("rxs"), ifstat.has("ptxs"), ifstat.has("txs") {
        let ppctstr =
          "(↓\(Self.flowPercent(ifstat, "prxs", "rxs"))% / ↑\(Self.flowPercent(ifstat, "ptxs", "txs"))% of flow)"
    ...
    let mlen = max(
      max(iaf.count, oaf.count, rxbStr.count, txbStr.count, ipf.count, opf.count),
  ...
  public func renderDiscoveredTable(_ interfaces: [DiscoveredInterfaceInfo]) -> String {
    ...
    for info in filtered(interfaces) {
      ...
      if difference < 60 {
        ...
        lastHeardDisplay = "\(Int(difference / 60))m ago"
      } else if difference < 86400 {
        lastHeardDisplay = "\(Int(difference / 3600))h ago"
      } else {
        lastHeardDisplay = "\(Int(difference / 86400))d ago"
      }
  ...
  public func renderDiscoveredDetails(_ interfaces: [DiscoveredInterfaceInfo]) -> String {
    ...
    for (index, info) in filtered(interfaces).enumerated() {
      ...
      let network =
        (!info.transportID.isEmpty && !info.networkID.isEmpty
      ...
      out +=
        "Discovered   : \(Self.prettytime(now - info.discovered, isFloat: true, compact: true)) ago\n"
      out +=
        "Last Heard   : \(Self.prettytime(now - info.lastHeard, isFloat: true, compact: true)) ago\n"
      out += "Location     : \(location)\n"

      if let frequency = info.frequency {
        out += "Frequency    : \(Self.thousands(frequency)) Hz\n"
      }
      if let bandwidth = info.bandwidth {
        out += "Bandwidth    : \(Self.thousands(bandwidth)) Hz\n"
      }
  ...
  static func flowPercent(
    _ ifstat: RNStatusInterfaceStats, _ numerator: String, _ denominator: String
  ) -> Int {
  ...
  static func aggregateBlock(
    label: String, stats: RNStatusStats,
    rxb: String, txb: String,
    rxs: String, txs: String,
    rxf: String, txf: String
  ) -> String {
    ...
    let rxstat =
      rxbStr + "  " + RNSUtilities.prettyspeed(stats.double(rxs))
      + ", \(percent(rxs, of: stats.rxs))% of flow, \(RNSUtilities.prettyfrequency(stats.double(rxf)))"
    let txstat =
      txbStr + "  " + RNSUtilities.prettyspeed(stats.double(txs))
  ...
  private static func optionalLine(
    _ label: String,
