...
public enum MultiprocessingAuth {
  ...
  public static func digestNameAndPayload(_ message: Data) throws -> (Digest?, Data) {
    ...
    var i = 1
    while i < searchEnd {
      if message[message.startIndex + i] == UInt8(ascii: "}") {
        closingIndex = i
        break
      }
      i += 1
  ...
  static func hmac(_ digest: Digest, key: Data, data: Data) -> Data {
    ...
    switch digest {
    case .md5:
      return Data(CryptoKit.HMAC<Insecure.MD5>.authenticationCode(for: data, using: symmetricKey))
    case .sha256:
      return Data(CryptoKit.HMAC<SHA256>.authenticationCode(for: data, using: symmetricKey))
    case .sha384:
      return Data(CryptoKit.HMAC<SHA384>.authenticationCode(for: data, using: symmetricKey))
    }
  }
