...
extension DispatchSourceProtocol {
    ...
    ///
    /// `cancel()` alone isn't enough, and getting this wrong kills the process rather than
    ...
    ///
    /// So an abandoned source must be resumed to clear its suspension, even though it never
    /// fires. `resume()` after `cancel()` is a no-op on delivery and legal—the last row of the preceding table.
    ///
