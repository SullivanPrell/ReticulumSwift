...
public final class RPCClient {
  ...
  public static func forInstance(
    storagePath: URL,
    host: String = "127.0.0.1",
    port: UInt16 = RPCClient.defaultControlPort,
    timeout: TimeInterval = 5
    ...
    RPCClient(
      host: host, port: port, authkey: try authkey(storagePath: storagePath), timeout: timeout)
  }
  ...
  public static func isSharedInstanceRunning(
    host: String = "127.0.0.1",
  ...
  public func nextHop(destinationHash: Data) throws -> Data? {
    if case .bytes(let hop) = try get(
      "next_hop", extra: [("destination_hash", .bytes(destinationHash))])
    {
      return hop
    }
    return nil
  ...
  public func nextHopInterfaceName(destinationHash: Data) throws -> String? {
    if case .string(let name) = try get(
      "next_hop_if_name", extra: [("destination_hash", .bytes(destinationHash))])
    {
      return name
    }
    return nil
  ...
  public func isBlackholed(identityHash: Data) throws -> Bool {
    if case .bool(let flag) = try get(
      "is_blackholed", extra: [("identity_hash", .bytes(identityHash))])
    {
      return flag
    }
    return false
  ...
  public func blackholeIdentity(
    _ identityHash: Data, until: TimeInterval? = nil, reason: String? = nil
  ) throws {
  ...
  private func handshake(_ fd: Int32) throws {
    ...
    guard
      MultiprocessingAuth.verifyChallenge(
        authkey: authkey,
        message: ourMessage,
