...
public final class NetworkProbe {
  ...
  public enum ValidationError: Error, Equatable {
    ...
    public var message: String {
      ...
      case .badHashLength:
        ...
        return
          "Destination length is invalid, must be \(NetworkProbe.destinationHexLength) hexadecimal characters (\(NetworkProbe.destinationHexLength / 2) bytes)."
      case .badHashHex:
  ...
  public static func appAndAspects(fromFullName fullName: String) -> (
    appName: String, aspects: [String]
  ) {
    let components = fullName.split(separator: ".", omittingEmptySubsequences: false).map(
      String.init)
  ...
  public static func effectiveTimeout(
    ...
    mediumPathTimeout: TimeInterval
  ) -> TimeInterval {
  ...
  public func run(options: Options) -> Result {
    ...
    if clock.now() > deadline {
      ...
      output.write(
        "\r" + String(repeating: " ", count: NetworkProbe.shortEraseWidth)
    ...
    do {
      requestDestination = try Destination(
    ...
    while remaining > 0 {
      ...
      do {
        ...
        output.write(
          "Error: Probe packet size of \(packedSize) bytes exceed MTU of \(Reticulum.mtu) bytes\n")
        output.flush()
        return .mtuExceeded
      } catch {
        receipt = nil
      ...
      output.write(
        "\rSent probe \(sent) (\(size) bytes) to "
      ...
      if clock.now() > deadline {
        ...
        output.write(
          "\r" + String(repeating: " ", count: NetworkProbe.longEraseWidth)
            + "\rProbe timed out\n")
        output.flush()
        outcomes.append(
          Outcome(
            index: sent, conclusion: .deadlineExceeded, rtt: nil,
        ...
        if let receipt, receipt.status == .delivered {
          ...
          output.write(
            "Valid reply from " + RNSUtilities.prettyhexrep(requestDestination.hash)
              + "\nRound-trip time is " + NetworkProbe.rttString(rtt)
              + " over \(hops) hop" + plural + stats + "\n\n")
          output.flush()
          outcomes.append(
            Outcome(
              index: sent, conclusion: .delivered, rtt: rtt,
          ...
          outcomes.append(
            Outcome(
              index: sent, conclusion: .receiptFailed, rtt: nil,
              hops: nil, receptionStats: "",
              destinationHashMismatch: hashMismatch))
        }
      }
    ...
    output.write(
      "Sent \(sent), received \(replies), packet loss "
