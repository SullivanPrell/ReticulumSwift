...
public final class RNPathRunner {
  ...
  private func runBlackholeAdd() -> RNPathApp.Result {
    ...
    do {
      ...
      let result = try management.blackholeIdentity(
        identityHash, until: until,
        reason: options.blackholeReason)
      // The confirmations echo the RAW hex as typed, not prettyhexrep.
      if result == true {
        output("Blackholed identity \(raw)")
      } else if result == nil {
        output("Identity \(raw) already blackholed")
      } else {
        output("Could not blackhole identity \(raw)")
      }
      return .ok
    } catch {
  ...
  private func runBlackholeLift() -> RNPathApp.Result {
    ...
    do {
      ...
      if result == true {
        output("Lifted blackhole for identity \(raw)")
      } else if result == nil {
        output("Identity \(raw) not blackholed")
      } else {
        output("Could not unblackhole identity \(raw)")
      }
      return .ok
    } catch {
  ...
  private func runTable() -> RNPathApp.Result {
    ...
    if let hex = options.destination {
      do { destinationHash = try RNPathApp.parseDestination(hex) } catch {
    ...
    if !remoteLinkPresent {
      do {
        ...
        table = RNPathTableEntry.sortedForDisplay(
          try management.pathTable(maxHops: options.maxHops))
      } catch {
        output("\(message(for: error))")
        return .generalFailure
      }
      ...
      let payload = RNPathRemoteClient.pathRequestPayload(
        command: RNPathApp.commandTable,
  ...
  private func runRates() -> RNPathApp.Result {
    ...
    if !remoteLinkPresent {
      do { table = try management.rateTable() } catch {
        output("\(message(for: error))")
        return .generalFailure
      }
    } else {
      ...
      let payload = RNPathRemoteClient.pathRequestPayload(
        command: RNPathApp.commandRates,
        destinationHash: destinationHash,
        maxHops: nil,
  ...
  private func runDropPath() -> RNPathApp.Result {
    ...
    output(
      "Unable to drop path to " + RNSUtilities.prettyhexrep(destinationHash) + ". Does it exist?")
    return .generalFailure
  }
  ...
  private func runDropVia() -> RNPathApp.Result {
    ...
    output(
      "Unable to drop paths via " + RNSUtilities.prettyhexrep(transportHash)
  ...
  private func runPathRequest() -> RNPathApp.Result {
    ...
    let deadline =
      now().timeIntervalSince1970 + max(options.timeout, management.mediumPathTimeout())
    while !resolver.hasPath(to: destinationHash), now().timeIntervalSince1970 < deadline {
      sleep(0.1)
      spinner?(
        "\u{8}\u{8}" + String(RNPathApp.spinnerSymbols[index % RNPathApp.spinnerSymbols.count])
          + " ")
    ...
    do { nextHopBytes = try management.nextHop(for: destinationHash) } catch {
    ...
    let interfaceName =
      ((try? management.nextHopInterfaceName(for: destinationHash)) ?? nil) ?? "None"

    output(
      RNPathFormatter.pathFoundLine(
        destinationHash: destinationHash,
        hops: hops,
        nextHop: nextHop,
  ...
  private func parsedDestinationOrFailure() -> Data? {
    do { return try RNPathApp.parseDestination(options.destination ?? "") } catch {
      output(message(for: error))
      return nil
    }
  }
