...
public final class RNXListener {
  ...
  public func register() {
    ...
    destination.registerNativeRequestHandler(
      path: RNXApp.requestPath,
      allow: .all
    ) { [weak self] _, value, requestID, link, _ in
  ...
  private func initiatorIdentified(_ link: Link, _ identity: Identity) {
    ...
    log(
      "Initiator of link \(RNSUtilities.prettyhexrep(link.linkID ?? Data())) "
  ...
  private func handleCommandRequest(_ value: MsgPack.Value, requestID: Data, link: Link) {
    ...
    executionQueue.async { [weak self] in
      ...
      if let remoteHash {
        self.log(
          "Delivering result of command [\(request.command)] to "
  ...
  public static func loadAllowedIdentitiesFile(
    ...
    for directory in searchPaths {
      let path =
        DaemonBootstrap.expandTilde(directory)
    ...
    where line.count == RNXApp.destinationHexLength {
