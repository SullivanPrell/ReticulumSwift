...
extension RNXRequest {
  ...
  public func packedValue() -> MsgPack.Value {
  ...
  public init(unpacking value: MsgPack.Value) throws {
    ...
    guard case .bytes(let commandBytes) = arr[0],
      let command = String(data: commandBytes, encoding: .utf8)
    else {
      throw RNXError.malformedRequest
    }
    self.init(
...
extension RNXResult {
  ...
  public func pack() -> Data { MsgPack.encode(packedValue()) }
