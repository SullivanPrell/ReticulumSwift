...
public struct RNStatusStats {
  ...
  public func sortedInterfaces(by sort: RNStatusApp.Sort?, reverse sortReverse: Bool)
    -> [RNStatusInterfaceStats]
  {
    ...
    let decorated = interfaces.enumerated().map {
      (offset: $0.offset, key: key($0.element), value: $0.element)
    }
  ...
  public static func shouldHide(_ stats: RNStatusInterfaceStats, showAll: Bool) -> Bool {
    let name = stats.name
    let i2pNonConnectable =
      name.hasPrefix("I2PInterface[")
  ...
  /// `-A`/`-P`.
  public static func passesFilters(
    _ stats: RNStatusInterfaceStats,
    nameFilter: String?,
    burstFilter: Bool
  ) -> Bool {
    let name = stats.name
    ...
    let burstActive =
      stats.has("burst_active") && stats.has("pr_burst_active")
