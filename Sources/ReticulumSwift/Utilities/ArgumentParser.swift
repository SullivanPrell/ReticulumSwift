...
public struct ArgumentParser {
  ...
  public mutating func option(
    _ names: [String], metavar: String = "VALUE",
  ...
  public mutating func variadic(
    _ names: [String], metavar: String = "VALUE",
    help: String, hidden: Bool = false
  ) {
    declarations.append(
      Declaration(
        names: names, kind: .variadic(metavar: metavar),
        help: help, hidden: hidden))
  }
  ...
  public mutating func optionalValue(
    _ names: [String], metavar: String = "VALUE",
  ...
  public mutating func appending(
    _ names: [String], metavar: String = "VALUE",
    help: String, hidden: Bool = false
  ) {
    declarations.append(
      Declaration(
        names: names, kind: .appended(metavar: metavar),
        help: help, hidden: hidden))
  }
  ...
  public func parse(_ arguments: [String]) throws -> ParsedArguments {
    ...
    while index < arguments.count {
      ...
      if !argument.hasPrefix("--"), argument.count > 2, let steps = shortCluster(argument) {
        for step in steps {
          ...
          case .value:
            let value: String
            if let attached = step.attached {
              value = attached
            } else if index < arguments.count {
              value = arguments[index]
              index += 1
            } else {
              throw ArgumentError.missingValue(step.key)
            }
          ...
          case .optionalValue(_, let const):
            if let attached = step.attached {
              values[step.key] = attached
            } else if index < arguments.count, !looksLikeOption(arguments[index]) {
              values[step.key] = arguments[index]
              index += 1
            } else if let const {
              values[step.key] = const
            }
          case .variadic:
    ...
    return ParsedArguments(
      flags: flags, counts: counts, values: values,
  ...
  public var optionSpecs: [OptionSpec] {
    declarations.map { declaration in
      ...
      case .flag, .counted:
        return OptionSpec(
          names: declaration.names, metavar: nil, nargs: .none,
          help: declaration.help, hidden: declaration.hidden)
      case .value(let metavar, _):
        return OptionSpec(
          names: declaration.names, metavar: metavar, nargs: .one,
          help: declaration.help, hidden: declaration.hidden)
      case .optionalValue(let metavar, _):
        return OptionSpec(
          names: declaration.names, metavar: metavar, nargs: .optional,
          help: declaration.help, hidden: declaration.hidden)
      case .variadic(let metavar):
        return OptionSpec(
          names: declaration.names, metavar: metavar, nargs: .any,
          help: declaration.help, hidden: declaration.hidden)
  ...
  public var usage: String {
    ...
    let positionalUsage =
      ...
      .joined(separator: " ")
    lines.append(
      "usage: \(program) [options] \(positionalUsage)".trimmingCharacters(in: .whitespaces))
    lines.append("")
    ...
    if !positionalNames.isEmpty {
      ...
      for positional in positionalNames {
        lines.append(
          "  \(positional.name.padding(toLength: max(24, positional.name.count + 2), withPad: " ", startingAt: 0))\(positional.help)"
        )
      }
    }
    ...
    lines.append(
      "  \("-h, --help".padding(toLength: 24, withPad: " ", startingAt: 0))show this help message and exit"
    )
...
public struct ParsedArguments {
  ...
  init(
    flags: Set<String>, counts: [String: Int], values: [String: String],
