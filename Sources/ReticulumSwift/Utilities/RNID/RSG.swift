...
public enum RSG {
  ...
  public static func validate(
    ...
    case .hash, .none:
      guard let pubkey = signedData.pubkey,
        let loaded = try? Identity(publicKeyBytes: pubkey)
    ...
    return ValidationResult(
      isValid: valid, signedData: signedData, signingIdentity: signingIdentity)
  }
  ...
  public static func checkReleaseRSMStructure(_ signedData: SignedData) -> String? {
    ...
    if nameText.contains("/") || versionText.contains("/") {
      return "Invalid data in release manifest"
    }
    ...
    if originLength != Constants.truncatedHashLength {
      return "Invalid origin hash length in manifest"
    }
