...
public final class RNIDOperations {
  ...
  public func printHashInformation(aspects dottedName: String) -> RNIDApp.Result {
    ...
    guard
      let destinationHash = RNIDDestinationHash.hash(
        fromFullName: dottedName,
        identityHash: identityHash)
    else {
      output.line(
        "An error ocurred while attempting to get hash information: "
          + "Invalid material supplied for destination hash calculation")
      return .unknownError
    }
    output.line(
      "The \(dottedName) destination for this Identity is "
  ...
  public func announce(aspects dottedName: String) -> RNIDApp.Result {
    ...
    do {
      ...
      output.line(
        "Announcing \(dottedName) destination "
  ...
  private func validateSingle(path: String) -> RNIDApp.Result {
    ...
    do {
      let result = try RSG.validate(
        rsgData: rsg, message: .file(reader),
      ...
      output.line(
        "Signature is valid, the file \(filePath) was signed by "
  ...
  private func validateLegacy(signaturePath: String, filePath: String) -> RNIDApp.Result {
    ...
    guard RSG.validateLegacy(signature: signature, fileData: fileData, identity: identity) else {
      output.line(
        "Invalid signature \(signaturePath) for file \(filePath)"
  ...
  private func validateMessage(path: String) -> RNIDApp.Result {
    ...
    do {
      ...
      let result = try RSG.validate(
        rsgData: rsg, message: .bytes(embedded),
        requiredSigner: requiredSigner)
      ...
      guard result.isValid,
        ...
        let signingIdentity = result.signingIdentity
      else {
  ...
  private func signSingle(path: String) -> RNIDApp.Result {
    ...
    do {
      if options.raw {
        ...
        let rsg = try RSG.create(
          signer: identity, message: .file(reader),
  ...
  public func signMessage(_ message: String?) -> RNIDApp.Result {
    ...
    do {
      let rsg = try RSG.create(
        signer: identity, message: body, embed: true,
  ...
  private func decryptSingle(path: String) -> RNIDApp.Result {
    ...
    let decryptPath =
      options.write.map { fileSystem.expandTilde($0) }
    ...
    do {
      ...
      try RNIDFileCrypto.decryptStream(identity: identity, reader: reader, writer: writer) {
        wrote in
  ...
  public func writeIdentity(path: String, exportPrivate: Bool) -> RNIDApp.Result {
    ...
    do {
      if identity.hasPrivateKey, exportPrivate {
        guard !fileSystem.fileExists(atPath: writePath) || options.force else {
