...
public final class RNIDIdentityResolver {
    ...
    -> Swift.Result<Identity?, RNIDApp.Result>
  {
    ...
    case .identityArgument(let value):
      return resolveIdentityArgument(
        value, allowNone: allowNone, noCache: noCache,
  ...
  private func resolveIdentityArgument(
    _ value: String,
    allowNone: Bool,
    noCache: Bool,
    request: Bool,
    ...
    -> Swift.Result<Identity?, RNIDApp.Result>
  {
    ...
    guard let requestedHash = RNIDEncoding.hexDecode(hexHash) else {
      output.line(
        "Invalid hexadecimal hash provided: non-hexadecimal number found in fromhex() arg")
      return .failure(.invalidIdentity)
    }
    ...
    if identity == nil {
      ...
      do {
        ...
        if let identityDestination = RNIDDestinationHash.hash(
          fromFullName: RNIDApp.defaultAspects,
      ...
      _ = pathWaiter?.wait(
        until: { self.recallEitherWay(requestedHash) != nil },
        message: message,
        timeout: timeout)
      ...
      identity = received
      output.line(
        "Received Identity \(RNIDRender.identity(received)) for destination "
  ...
  private func importIdentity(_ value: String, isPrivate: Bool)
    -> Swift.Result<Identity?, RNIDApp.Result>
  {
    ...
    // (4) Base64 (URL-safe).
    if identityBytes == nil, let decoded = RNIDEncoding.base64URLDecode(value),
      decoded.count == size
    {
      identityBytes = decoded
