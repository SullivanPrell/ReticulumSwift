...
public final class LocalManagementSource: RNPathManagementSource {
  ...
  public func blackholeIdentity(_ identityHash: Data, until: TimeInterval?, reason: String?) throws
    -> Bool?
  {
    // Reticulum.blackholeIdentity takes a `Date?`, not a `TimeInterval?`.
    reticulum.blackholeIdentity(
      identityHash,
...
extension RNPathApp {
  ...
  public static func managementSourceKind(
    role: InstanceConnection.Role,
    hasRPC: Bool
  ) -> ManagementSourceKind {
    switch role {
  ...
  public static func makeManagementSource(for connection: InstanceConnection)
    -> RNPathManagementSource
  {
