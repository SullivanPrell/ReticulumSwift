...
/// - RESOURCE data parts aren't link-encrypted (resource pre-encrypts).
...
/// ## Thread safety
///
/// A single non-recursive `stateLock` guards all mutable transfer state
...
public final class ResourceTransfer {
  ...
  public private(set) var advertisement: ResourceAdvertisement? {
    get {
      stateLock.lock()
      defer { stateLock.unlock() }
      return unsafeAdvertisement
    }
    set {
      stateLock.lock()
      unsafeAdvertisement = newValue
      stateLock.unlock()
    }
  }
  ...
  public private(set) var resourceHash: Data {
    get {
      stateLock.lock()
      defer { stateLock.unlock() }
      return unsafeResourceHash
    }
    set {
      stateLock.lock()
      unsafeResourceHash = newValue
      stateLock.unlock()
    }
  }
  ...
  /// each segment—so this equals ``progress``. It exists as a distinct name
  ...
  /// Swift's non-throwing closure provides that for free.
  ...
  public private(set) var receivedMetadata: Data? {
    get {
      stateLock.lock()
      defer { stateLock.unlock() }
      return unsafeReceivedMetadata
    }
    set {
      stateLock.lock()
      unsafeReceivedMetadata = newValue
      stateLock.unlock()
    }
  }
  ...
  private func watchdogTick() {
    stateLock.lock()
    if unsafeStatus.isTerminal {
      stateLock.unlock()
      stopWatchdog()
      return
    }
    let sinceActivity = Date().timeIntervalSince(lastActivity)
    guard sinceActivity >= retryTimeout else {
      stateLock.unlock()
      return
    }
  ...
  /// disagrees means the peer sized its parts against a different MTU, and the transfer can't
  /// complete however long it's left running.
  ...
  /// Called by Link when a RESOURCE_REQ arrives for this resource hash.
  internal func handleRequest(_ data: Data) {
    ...
    stateLock.lock()
    guard
      unsafeStatus == .advertised || unsafeStatus == .transferring || unsafeStatus == .awaitingProof
    else {
      stateLock.unlock()
      return
    }
    ...
    let sendEnd = min(
      receiverMinConsecutiveHeight + ResourceAdvertisement.collisionGuardSize, mapHashes.count)
    ...
    if wantsMoreHashmap, data.count >= 1 + ResourceTransfer.mapHashLength {
      ...
      if partIndex % hml != 0 {
        // Next segment isn't aligned to a HASHMAP_MAX_LEN boundary—the
  ...
  /// Called by Link when a RESOURCE_PRF proof arrives for this resource hash.
  ...
  /// receive and `adv` is the segment it's waiting for.
  ///
  ...
  ///
  /// A transfer that hasn't yet buffered a segment takes anything (it's
  /// fresh, or Link picked it deliberately). One that's mid-multi-segment
  ...
  internal func receiveAdvertisement(
    _ data: Data,
    ...
    // same overall resource, and the awaited segment.
    guard acceptsAsContinuation(adv) else { return }

    // Derive the part count rather than reading it off the wire.
    //
    // `bugs/016`. Python doesn't trust the advertisement—it computes
    ...
    let derived = ResourceTransfer.derivedPartCount(
      size: Int(adv.transferSize),
      segmentSize: sdu)
    ...
    if ResourceTransfer.partCountDisagrees(advertised: Int(adv.partCount), derived: derived) {
      Reticulum.log(
        "Resource \(RNSUtilities.hexrep(adv.resourceHash, delimit: false)) "
  ...
  internal func receivePart(_ data: Data) {
    stateLock.lock()
    guard unsafeStatus == .transferring else {
      stateLock.unlock()
      return
    }
    lastActivity = Date()
    ...
    let partHash = Hashes.fullHash(data + randomHashForReceiverLocked()).prefix(
      ResourceTransfer.mapHashLength)
  ...
  private func assemble() {
    ...
    guard computedHash == adv.resourceHash else {
      fail(
        "assembly: hash mismatch (computed=\(computedHash.hexString) expected=\(adv.resourceHash.hexString) compressed=\(adv.compressed) plaintext=\(assembledPlaintext.count)B)"
      )
      return
    }
    ...
    if adv.hasMetadata, adv.segmentIndex == 1, assembledPlaintext.count >= 3 {
      let sz =
        Int(assembledPlaintext[0]) << 16 | Int(assembledPlaintext[1]) << 8
        | Int(assembledPlaintext[2])
    ...
    if segIdx < segTotal {
      ...
      // to this object directly—don't unregister + re-register since that
  ...
  private func fail(_ reason: String) {
    ...
    if link.status == .active, !rhash.isEmpty {
      let cancelContext: Packet.Context =
        receiver ? .resourceReceiverCancel : .resourceInitiatorCancel
