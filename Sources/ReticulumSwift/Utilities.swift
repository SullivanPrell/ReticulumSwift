...
public enum RNSUtilities {
  ...
  public static func prettytime(_ time: TimeInterval, verbose: Bool = false, compact: Bool = false)
    -> String
  {
    var t = time
    let neg = t < 0
    ...
    t = t.truncatingRemainder(dividingBy: 86400)
    let hours = Int(t / 3600)
    t = t.truncatingRemainder(dividingBy: 3600)
    let minutes = Int(t / 60)
    t = t.truncatingRemainder(dividingBy: 60)
  ...
  public static func prettyshorttime(
    _ time: TimeInterval, verbose: Bool = false, compact: Bool = false
  ) -> String {
    var t = time
    let neg = t < 0
    if neg { t = -t }
  ...
  public static func prettyfrequency(
    _ hz: Double, suffix: String = "Hz", d: Int = 2, lpf: Bool = false
  ) -> String {
    ...
    for unit in units {
      if abs(num) < 1000.0 {
        guard d == 2 else {
          return "\(round(num * pow(10, Double(d))) / pow(10, Double(d))) \(unit)\(suffix)"
        }
