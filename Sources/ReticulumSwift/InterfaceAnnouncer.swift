...
public final class InterfaceAnnouncer {
  ...
  public func tick() {
    ...
    guard let appData = announceData(for: selected) else {
      Reticulum.log(
        "Could not generate interface discovery announce data for "
          + selected.name, level: .error)
      return
    }

    Reticulum.log(
      "Sending interface discovery announce for \(selected.name) with "
        + "\(appData.count)B payload", level: .debug)
    do { _ = try transport.announce(destination: discoveryDestination, appData: appData) } catch {
      Reticulum.log(
        "Error while sending interface discovery announce: \(error)",
        level: .error)
    }
  }
  ...
  public func announceData(for interface: any Interface) -> Data? {
    ...
    var info: [(MsgPack.Value, MsgPack.Value)] = [
      ...
      (
        .uint(DiscoveryFieldKey.longitude.rawValue),
        Self.optionalDouble(interface.discoveryLongitude)
      ),
    ...
    if interfaceType == "TCPClientInterface" {
      Reticulum.log(
        "Invalid interface discovery configuration for \(interface.displayName), "
    ...
    case "BackboneInterface", "TCPServerInterface":
      ...
      guard
        InterfaceDiscoveryHelpers.isIPAddress(resolved)
          || InterfaceDiscoveryHelpers.isHostname(resolved)
      else {
        Reticulum.log(
          "The configured reachable_on parameter \"\(resolved)\" for "
      ...
      info.append(
        (
          .uint(DiscoveryFieldKey.port.rawValue),
    ...
    case "RNodeInterface":
      ...
      info.append(
        (
          .uint(DiscoveryFieldKey.frequency.rawValue),
          Self.optionalInt(radio?.frequency)
        ))
      info.append(
        (
          .uint(DiscoveryFieldKey.bandwidth.rawValue),
          Self.optionalInt(radio?.bandwidth)
        ))
      info.append(
        (
          .uint(DiscoveryFieldKey.spreadingFactor.rawValue),
          Self.optionalInt(radio?.spreadingFactor)
        ))
      info.append(
        (
          .uint(DiscoveryFieldKey.codingRate.rawValue),
    ...
    case "WeaveInterface":
      ...
      info.append(
        (
          .uint(DiscoveryFieldKey.channel.rawValue),
          Self.optionalInt(interface.discoveryChannel)
        ))
      info.append(
        (
          .uint(DiscoveryFieldKey.modulation.rawValue),
    ...
    if let cached {
      ...
      guard
        let found = stampGenerator.generateStamp(
          material: infohash, targetCost: cost,
          expandRounds: Self.workblockExpandRounds)
      else { return nil }
      stamp = found
      lock.lock()
      stampCache[infohash] = found
      lock.unlock()
    }
    ...
    if interface.discoveryEncrypt {
      ...
      guard let networkIdentity = transport?.networkIdentity else {
        Reticulum.log(
          "Discovery encryption requested for \(interface.displayName), but "
  ...
  private func resolveReachableAddress(_ configured: String) -> String? {
    ...
    case .failed(let error):
      Reticulum.log(
        "Error while getting reachable_on from executable at \(sanitized): "
  ...
  private func applyLocationCommand(to interface: any Interface) -> Bool {
    ...
    case .failed(let error):
      Reticulum.log(
        "Error while getting discovery location from executable at "
