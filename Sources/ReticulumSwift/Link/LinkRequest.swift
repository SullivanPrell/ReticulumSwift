...
extension Link {
    ...
    func handleIncomingResponse(_ data: Data) {
        ...
        // Python: `transfer_size = len(umsgpack.packb(response_data))-2`, passed as both
        // response_size and response_transfer_size with update_sizes=True (Link.py:998-999).
        // Without this both stayed nil, so rnx's -d "Transferred N bytes … effective rate"
        // line had nothing to report.
        let transferSize = max(0, MsgPack.encode(parts[1]).count - 2)
