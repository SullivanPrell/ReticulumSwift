...
public final class Transport {
  ...
  public var transportInstanceID: Data = {
    SecureRandom.bytes(Constants.truncatedHashLength)
  }()
  ...
  public func register(announceHandler: any AnnounceHandler) {
    ...
    defer { announceHandlerLock.unlock() }
  ...
  @discardableResult
  func registerInflightPathRequest(
  ...
  private func localClientServingInterfaces(excluding excluded: (any Interface)?) -> [any Interface]
  {
    interfaces.filter { iface in
      guard let serving = iface as? any LocalClientServingInterface, serving.clientCount > 0 else {
        return false
      }
  ...
  public func isAnnounceRateBlocked(
    destinationHash: Data,
    ...
    guard now > entry.blockedUntil else {
    ...
    guard entry.violations > interface.announceRateGrace else {
      entry.last = now
      announceRateTable[destinationHash] = entry
      return false
    }
  ...
  public func shouldIngressLimit(
    ...
    guard var state = ingressStates[key],
      let tracker = tracker(for: interface)
    else { return false }

    let age = now - interface.createdAt.timeIntervalSince1970
    let threshold =
      age < interface.interfaceState.icNewTime
  ...
  public func shouldIngressLimitPR(
    on interface: any Interface,
    now: TimeInterval = Date().timeIntervalSince1970
  ) -> Bool {
    guard interface.ingressControl else { return false }
    ...
    guard state.prBurstActive else {
      if freq > threshold {
  ...
  public func shouldEgressLimitPR(
    on interface: any Interface,
    now: TimeInterval = Date().timeIntervalSince1970
  ) -> Bool {
  ...
  @discardableResult
  public func processHeldAnnounces(
    for interface: any Interface,
    now: TimeInterval = Date().timeIntervalSince1970
  ) -> Packet? {
    let key = ObjectIdentifier(interface)
  ...
  public func sampleInterfaceSpeeds(now: TimeInterval = Date().timeIntervalSince1970) {
    ...
    let counters = snapshot.map {
      iface -> (any Interface, InterfaceFreqTracker.Counts?, (Double, Double, Double, Double)) in
      let tr = tracker(for: iface)
      let freqs = (
        tr?.incomingAnnounceFrequency(now: now) ?? 0,
    ...
    var totalAnnounceRxSpeed: Double = 0
    var totalAnnounceTxSpeed: Double = 0
    var totalPrRxSpeed: Double = 0
    var totalPrTxSpeed: Double = 0
    var totalAnnounceRxFreq: Double = 0
    var totalAnnounceTxFreq: Double = 0
    var totalPrRxFreq: Double = 0
    var totalPrTxFreq: Double = 0
    for (iface, counts, freqs) in counters {
      ...
      let sample = SpeedSample(
        rxBytes: currentRx, txBytes: currentTx,
  ...
  public func notifyIncomingAnnounce(
    on interface: any Interface, at t: TimeInterval,
    size: Int = 0
  ) {
  ...
  public func notifyOutgoingAnnounce(
    on interface: any Interface, at t: TimeInterval,
    size: Int = 0
  ) {
  ...
  public func notifyIncomingPathRequest(
    on interface: any Interface, at t: TimeInterval,
    size: Int = 0
  ) {
  ...
  public func notifyOutgoingPathRequest(
    on interface: any Interface, at t: TimeInterval,
    size: Int = 0
  ) {
  ...
  public func interfaceCounts(for interface: any Interface) -> InterfaceFreqTracker.Counts {
    tracker(for: interface)?.counts()
      ?? InterfaceFreqTracker.Counts(
        announceRxBytes: 0, announceTxBytes: 0,
  ...
  func sweepPendingRestores(now: Date = Date()) {
    ...
    guard let readAt = pendingRestoresReadAt,
      now.timeIntervalSince(readAt) > Transport.pendingRestoreWindow
    else { return }
    if !pendingPathRestores.isEmpty {
      Reticulum.log(
        "Giving up on \(pendingPathRestores.count) restored path table "
  ...
  public static func configureIfac(
    ...
    let key = HKDF.derive(
      length: Constants.keySize, derivedFrom: originHash, salt: Constants.ifacSalt)
  ...
  public func restore(
    identity: Identity, forDestination destinationHash: Data, announcedAt: Date = Date()
  ) {
    lock.lock()
    defer { lock.unlock() }
  ...
  public func sweepKnownRatchets(now: Date = Date()) {
    lock.lock()
    defer { lock.unlock() }
    for (hash, received) in knownRatchetTimes where now.timeIntervalSince(received) > ratchetExpiry
    {
  ...
  private func persistKnownRatchet(_ ratchet: Data, forDestination hash: Data, receivedAt: Date) {
    ...
    try? MsgPack.encode(payload).write(
      to: dir.appendingPathComponent(hash.hexString),
      options: .atomic)
  }
  ...
  public func saveKnownDestinations(to url: URL) throws {
    ...
    let pairs: [(MsgPack.Value, MsgPack.Value)] = snapshot.map { destHash, identity in
      let lastUse = Transport.lastUseValue(
        retained: retained.contains(destHash),
        lastUsed: lastUsed[destHash])
      return (
        .bytes(destHash),
        .array([
          .double(
            announcedAt[destHash]?.timeIntervalSince1970
        ...
        appName: "rnstransport", aspects: ["remote", "management"])
      {
        ...
        mgmt.registerRequestHandler(path: "/path", allow: .list, allowedList: allowed) {
          ...
          guard let data, case .array(let arr) = (try? MsgPack.decode(data)) ?? .nil,
            !arr.isEmpty, case .string(let command) = arr[0]
          else { return nil }
          ...
          case "table":
            ...
            let filtered =
              filterHash == nil ? table : table.filter { $0.destinationHash == filterHash }
            let entries = filtered.map { e -> MsgPack.Value in
              .map([
                (.string("hash"), .bytes(e.destinationHash)),
          ...
          case "rates":
            ...
            let filtered =
              filterHash == nil ? rates : rates.filter { $0.destinationHash == filterHash }
            let entries = filtered.map { r -> MsgPack.Value in
              .map([
                (.string("hash"), .bytes(r.destinationHash)),
  ...
  public func setupNetworkDestinations() {
    ...
    if let instanceDest = try? Destination(
      identity: netIdentity, direction: .in, kind: .single,
      appName: "rnstransport",
      aspects: ["network", "instance", hexHash])
    {
      register(destination: instanceDest)
    }
    if let netDest = try? Destination(
      identity: netIdentity, direction: .in, kind: .single,
  ...
  private func startJobsLoop() {
    ...
    timer.schedule(
      deadline: .now() + Transport.jobInterval,
  ...
  func processAnnounceRetries(now: TimeInterval = Date().timeIntervalSince1970) {
    ...
    for (destinationHash, stored) in announceTable {
      ...
      if entry.retries > 0 && entry.retries >= Transport.localRebroadcastsMax {
        ...
        entry.retransmitTimeout =
          now + Transport.pathfinderG
  ...
  private func retransmitAnnounce(
    _ entry: AnnounceTableEntry, interfaces snapshot: [any Interface],
    now: TimeInterval
  ) {
    ...
    where iface.isOnline && iface.isRoutingEndpoint
  ...
  static func shouldGenerateReceipt(for packet: Packet) -> Bool {
    ...
    if context >= Packet.Context.resource.rawValue,
      context <= Packet.Context.resourceReceiverCancel.rawValue
    {
      return false
    }
    return true
  ...
  public func send(_ packet: Packet, generateReceipt: Bool = true) throws -> PacketReceipt? {
    ...
    if packet.destinationType == .link {
      lock.lock()
      let link = links[packet.destinationHash]
      lock.unlock()
    ...
    if generateReceipt, Transport.shouldGenerateReceipt(for: packet) {
      if let hashable = try? packet.hashablePart() {
        ...
        lock.lock()
        let peerIdentity =
          knownIdentities[packet.destinationHash]
    ...
    lock.lock()
    let localDest =
      packet.destinationType == .single ? registeredDestinations[packet.destinationHash] : nil
  ...
  public func send(_ packet: Packet, exceptInterface excluded: Interface) {
    ...
    where interface.isOnline && interface.isRoutingEndpoint && interface !== excluded {
  ...
  public func announce(
    ...
    let packet = try Announce.make(
      for: destination, appData: appData, ratchet: ratchet, isPathResponse: isPathResponse)
  ...
  func handleIncoming(packet: Packet, from interface: Interface) {
    ...
    if let pktHash {
      if let rssi = packet.rssi {
        ...
        if packetRssiCache.count > Transport.localClientCacheMaxSize {
          packetRssiCache.removeFirst()
        }
      }
  ...
  private func handleLinkRequest(_ packet: Packet, from interface: Interface) {
    ...
    guard let linkIDHashable = try? Link.linkIDHashable(for: packet, dataLength: packet.data.count)
    else {
      return
    }
  ...
  private func clampRelayedLinkRequestMtu(
  ...
  private func forwardLinkTraffic(_ packet: Packet, from sourceInterface: Interface) {
    ...
    let touchesLocalClient =
      (initIface.map(isLocalClientInterface) ?? false)
    ...
    lock.lock()
    linkRoutes[packet.destinationHash] = route
    lock.unlock()
  }
  ...
  private func handleAnnounce(_ packet: Packet, from interface: Interface) {
    ...
    do {
      ...
      let rateBlocked = isAnnounceRateBlocked(
        destinationHash: decoded.destinationHash,
        interface: interface)
  ...
  public func savePacketHashlist(to url: URL) throws {
    ...
    for hash in snapshot {
      ...
      guard hash.count == Constants.fullHashLength else {
        skipped += 1
        continue
      }
      raw.append(hash)
    ...
    if skipped > 0 {
      Reticulum.log(
        "Skipped \(skipped) packet hashlist entr(ies) that are not "
  ...
  public func provePacket(
    _ packet: Packet, from sourceInterface: any Interface, destination: Destination
  ) {
  ...
  private func sendProof(
    for packet: Packet, from sourceInterface: Interface, destination: Destination
  ) {
    ...
    let proofData: Data =
      Reticulum.shouldUseImplicitProof()
