...
/// still can't read—the worst of the available outcomes, because it reads as correct in a
/// directory listing.
///
/// That announce reference is also why the announce cache had to be brought to parity *first*: an
/// entry whose announce can't be loaded is discarded whole (`Transport.py:334-345`), so a correct
...
public struct PathStore {
  ...
  public struct Entry {
    ...
    public static func decode(_ value: MsgPack.Value) -> Entry? {
      guard case .array(let f) = value, f.count == 8,
        ...
        case .bytes(let announceHash) = f[7]
      else { return nil }
  ...
  public static func snapshot(of transport: Transport) -> PathStore {
    ...
    for (destHash, path) in paths {
      ...
      // `Transport.py:3374`. A path whose interface is gone can't be resolved back on
      // load, and writing it down would leave a route pointing at nothing.
      guard let interface = path.nextHopInterface,
        liveInterfaceHashes.contains(interface.hash)
      else {
        Reticulum.log(
          "Skipping persist for path table entry "
      ...
      // discards exactly this case on load (`Transport.py:334`)—so it isn't written.
  ...
  ///
  /// Returns `nil` when the cache holds no packet for that hash or it won't unpack; the
  ...
  /// packet from cache is equivalent to receiving it again over an interface and it's cached
  /// with its non-increased hop count (`Transport.py:337-339`, and the comment at `:2640-2644`).
  /// The hop count isn't part of a packet's hashable part, so the increment doesn't move the
  /// announce hash.
  public static func restoredAnnounce(for entry: Entry, from transport: Transport) -> Packet? {
    guard
      var announce = (try? transport.getCachedAnnounce(hash: entry.announceHash))
        ?? nil
    else { return nil }
    announce.hops &+= 1
    return announce
  ...
  /// dropped every entry, every time (`bugs/041`). `apply(to:)` therefore parks what it can't
  ...
  /// which is the reference's outcome for an interface that isn't there.
  ...
  func install(into transport: Transport) -> Set<Data> {
    ...
    for entry in entries {
      ...
      // (`Transport.py:326,334`). A route that can't name the interface it goes through is
      ...
      guard let interfaceHash = entry.interfaceHash,
        let interface = transport.findInterface(fromHash: interfaceHash)
      else {
      ...
      // `announce_packet.packet_hash`—a value the increment can't change, since hops is
      ...
      // through a path entry that can't show it.
      guard PathStore.restoredAnnounce(for: entry, from: transport) != nil else {
        Reticulum.log(
          "Could not reconstruct path table entry from storage for "
  ...
  ///
  /// A file that doesn't decode throws, so the caller starts with an empty table and logs it,
  /// as the reference does at `:357-359`. Individual entries that don't have the reference's
