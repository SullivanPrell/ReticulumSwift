...
/// Per-interface announce queue.
///
/// Buffers forwarded announces when the
...
final class AnnounceQueue {
  ...
  /// Per-interface backlog cap.
  ///
  /// Matches Python's `Reticulum.MAX_QUEUED_ANNOUNCES
  ...
  /// Destination hashes queued, for O(1) duplicate detection.
  ///
  /// Kept in
  ...
  /// Guards `entries`, `queuedDests`, and `allowedAt`.
  ///
  /// The Transport fetches a
  ...
  /// Attempt to transmit `packet` now.
  ///
  /// If the interface is rate-limited,
