...
extension Transport {
  ...
  public func persistBlacklist(toDirectory directory: URL) throws {
    ...
    let localFile = directory.appendingPathComponent(
      StorageInventory.Entry.blackholeLocal.fileName)
    let tmpFile = directory.appendingPathComponent(
      StorageInventory.Entry.blackholeLocalTemp.fileName)
    ...
    for filename in files {
      ...
      if filename == "local" {
        ...
        guard let decoded = Data(hex: filename),
          decoded.count == Constants.truncatedHashLength
        else { continue }
      ...
      for (key, value) in entries {
        guard case .bytes(let hash) = key,
          hash.count == Constants.truncatedHashLength
        else { continue }
        ...
        if let existing = blackholedIdentities[hash],
          existing.source == ownHash, src != ownHash
        {
          continue
        }
