...
public enum MsgPack {
  ...
  private static func writeUInt(_ n: UInt64, into out: inout Data) {
    if n <= 0x7F {
      ...
      out.append(0xCD)
      appendBigEndian(UInt16(n), byteCount: 2, into: &out)
    } else if n <= UInt64(UInt32.max) {
      out.append(0xCE)
      appendBigEndian(UInt32(n), byteCount: 4, into: &out)
    } else {
      out.append(0xCF)
      appendBigEndian(n, byteCount: 8, into: &out)
    }
  }

  private static func writeInt(_ n: Int64, into out: inout Data) {
    if n >= 0 {
      writeUInt(UInt64(n), into: &out)
      return
    }
    if n >= -32 {
  ...
  private static func read(_ data: Data, cursor: inout Int, depth: Int = 0) throws -> Value {
    ...
    case 0xC4:
      return try readBin(
        byteCount: Int(try readUInt(8, data, &cursor)), data: data, cursor: &cursor)
    case 0xC5:
      return try readBin(
        byteCount: Int(try readUInt(16, data, &cursor)), data: data, cursor: &cursor)
    case 0xC6:
      return try readBin(
        byteCount: Int(try readUInt(32, data, &cursor)), data: data, cursor: &cursor)
    case 0xCA:
    ...
    case 0xDC:
      return try readArray(
        count: Int(try readUInt(16, data, &cursor)), data: data, cursor: &cursor, depth: depth)
    case 0xDD:
      return try readArray(
        count: Int(try readUInt(32, data, &cursor)), data: data, cursor: &cursor, depth: depth)
    case 0xDE:
      return try readMap(
        count: Int(try readUInt(16, data, &cursor)), data: data, cursor: &cursor, depth: depth)
    case 0xDF:
  ...
  private static func readArray(count: Int, data: Data, cursor: inout Int, depth: Int) throws
    -> Value
  {
    var values: [Value] = []
...
extension MsgPack.Value {
  ...
  public var asArray: [MsgPack.Value]? {
  ...
  public var asDictionary: [String: MsgPack.Value]? {
