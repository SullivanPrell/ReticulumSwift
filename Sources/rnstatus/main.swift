...
let blockedIPs = arguments.flag("--blocked-ips")
let trafficTotals = arguments.flag("--totals")
let packetsPerSecond = arguments.flag("--pps")
let queueStats = arguments.flag("--queues")
...
let monitorInterval = requiredDouble(
  "--monitor-interval", default: RNStatusApp.defaultMonitorInterval)
...
options.blockedIPs = blockedIPs
options.trafficTotals = trafficTotals
options.pps = packetsPerSecond
options.queueStats = queueStats
...
if discoveredMode {
  ...
  func renderDiscovered() -> String {
    ...
    return detailsRequested
      ? renderer.renderDiscoveredDetails(discovered)
...
if let remoteHex {
  ...
  guard let identityPath = managementIdentity else {
    fail(
      "Remote management requires an identity file. Use -i to specify the path to a management identity.",
      .remoteError)
  }
  guard remoteHex.count == RNStatusApp.destinationHexLength else {
    fail(
      "Destination length is invalid, must be \(RNStatusApp.destinationHexLength) hexadecimal characters "
...
func snapshot() -> Snapshot? {
  if let remoteQuery {
    ...
    do {
      let (stats, linkCount) = try remoteQuery.requestBlocking(
  ...
  return Snapshot(
    stats: InterfaceStatsPayload.build(connection.reticulum.transport),
...
func renderOnce() -> (text: String, code: RNStatusApp.Result) {
  guard let snapshot = snapshot(), let decoded = RNStatusStats(snapshot.stats) else {
    ...
    guard remoteHex == nil else {
      return (
        "Could not get RNS status from remote transport instance "
