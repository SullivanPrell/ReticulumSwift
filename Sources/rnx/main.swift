...
final class Session {
  ...
  func recordProgress(_ progress: Double, transferSize: Int) {
    statsLock.lock()
    stats.record(
      progress: progress, transferSize: transferSize,
...
func spinStat(until: () -> Bool, timeout: TimeInterval?) -> Bool {
  ...
  while Date().timeIntervalSince1970 < (deadline ?? .infinity) && !until() {
    ...
    Thread.sleep(forTimeInterval: 0.1)
    writeOut(
      "\r" + blank + "\rReceiving result " + String(symbols[index]) + " "
        + session.statusLine + " ")
    fflush(stdout)  // Python: sys.stdout.flush()
    index = (index + 1) % symbols.count
  }
  ...
  if !options.noAuth {
    for value in options.allowed {
      do {
        ...
        print(
          "Allowed destination length is invalid, must be \(hexLength) "
...
func runClient(_ options: Options, command: String, stdin: String?, interactive: Bool) -> Int? {
  ...
  if !spin(
    until: { client.linkStatus == .active },
  ...
  let rexecTimeout = RNXClient.rexecTimeout(
  ...
  spin(
    until: { !RNXClient.isDelivered(receipt.status) },
  ...
  let metrics = RNXResultRenderer.Metrics(
    requestSize: receipt.requestSize,
...
struct RNXMain {
  static func main() {
    ...
    if options.listen || options.printIdentity {
      ...
      runClient(
        options, command: command, stdin: options.stdin,
